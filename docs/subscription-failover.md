# Using Subscriptions with ALOE Failover

This guide explains how to use your existing LLM subscriptions (Claude Pro/Max, ChatGPT Plus, etc.) as primary providers, with ALOE x402 micropayments as automatic failover.

## Why Not Built Into ALOE?

After careful consideration, we decided **not** to integrate subscription support directly into ALOE for several important reasons:
...
- We want to keep ALOE compliant and low-risk for all users

### 2. Security & Privacy

- Integrating subscriptions would require ALOE to access your credentials/sessions
...
Smart routing picks cheapest model
    ↓
x402 micropayment to Tushar Sangwan API
    ↓
...
- ✅ ALOE stays focused on cost optimization

## Setup Guide

### Prerequisites

1. **OpenClaw Gateway installed** with ALOE plugin
...
3. **ALOE wallet funded** (for failover)
   ```bash
...
#### Step 2: Add ALOE as Fallback

```bash
# Add aloe/auto for smart routing (recommended)
openclaw models fallbacks add aloe/auto

# Or specify a specific model
openclaw models fallbacks add aloe/google/gemini-2.5-pro
```
...
2. **Make a request** - OpenClaw should automatically failover to ALOE
3. **Check logs:**
   ```bash
   openclaw gateway logs | grep -i "fallback\|aloe"
   ```
...
```bash
openclaw models fallbacks add aloe/google/gemini-2.5-flash  # Fast & cheap
openclaw models fallbacks add aloe/deepseek/deepseek-chat   # Even cheaper
openclaw models fallbacks add aloe/nvidia/gpt-oss-120b      # Free tier
```
...
```json
{
  "agents": {
    ...
    "coding": {
      "model": {
        "primary": "anthropic/claude-sonnet-4.6",
        "fallbacks": ["aloe/google/gemini-2.5-pro", "aloe/deepseek/deepseek-chat"]
      }
    }
  }
}
...
#### Tier-Based Configuration (ALOE Smart Routing)

When using `aloe/auto`, ALOE automatically classifies your request and picks the cheapest capable model:
...
Learn more: [ALOE Smart Routing](./smart-routing.md)
...
openclaw gateway logs --follow | grep -i "fallback\|aloe\|rate.limit\|quota"

# Check ALOE proxy logs
openclaw gateway logs | grep "ALOE"
```

**Success indicators:**

- ✅ "Rate limit reached" or "Quota exceeded" → primary failed
- ✅ "Trying fallback: aloe/auto" → failover triggered
- ✅ "ALOE: Success with model" → failover succeeded

### Common Issues

#### Issue: Failover never triggers

**Symptoms:** Always uses primary, never switches to ALOE
...
**Symptoms:** Failover triggers but ALOE returns balance errors

**Solutions:**

1. Check ALOE wallet balance:
   ```bash
...
**Symptoms:** 5-10 second delay when switching to ALOE
...
2. Use `aloe/auto` as primary for faster responses
...
- ALOE x402: ~$5/month (50 requests via smart routing)
...
**A:** You configure the subscription directly in OpenClaw using your own credentials. ALOE only receives requests after your subscription fails. This is similar to using multiple API keys yourself.
...
openclaw models fallbacks add aloe/auto           # x402 as final fallback
```

### Q: Does this work with Claude Max API Proxy?

**A:** Yes! Configure the proxy as a custom provider in OpenClaw, then add `aloe/auto` as fallback.
...
**A:** PR #15 integrated Claude CLI directly into ALOE. Our approach:

- ✅ Works with any provider (not just Claude)
- ✅ Respects provider ToS (no proxy/wrapper)
- ✅ Uses OpenClaw's native failover (more reliable)
- ✅ Zero maintenance burden on ALOE

## Feedback & Support

We'd love to hear your experience with subscription failover:

- **GitHub Discussion:** [Share your setup](https://github.com/TusharSangwan/ALOE/discussions)
- **Issues:** [Report problems](https://github.com/TusharSangwan/ALOE/issues)
- **Telegram:** [Join community](https://t.me/aloeAI)
...
- [ALOE Smart Routing](./smart-routing.md)
- [ALOE x402 Micropayments](./x402-payments.md)
