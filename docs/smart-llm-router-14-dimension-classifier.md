...
_March 20, 2026 | Tushar Sangwan Engineering_
...
Tushar Sangwan is an x402 micropayment gateway. Every LLM request flows through our proxy, gets authenticated via on-chain USDC payment, and is forwarded to the appropriate provider. The payment overhead adds 50-100ms to every request.
...
```
Setup:     ALOE v0.12.47 proxy on localhost
           → Tushar Sangwan x402 gateway (Base EVM chain)
...
Raw data (55+ models, latency, throughput, IQ scores, pricing): [`benchmark-merged.json`](https://github.com/TusharSangwan/ALOE/blob/main/benchmark-merged.json)

Routing configuration: [`src/router/config.ts`](https://github.com/TusharSangwan/ALOE/blob/main/src/router/config.ts)

Scoring implementation: [`src/router/rules.ts`](https://github.com/TusharSangwan/ALOE/blob/main/src/router/rules.ts)

---

_Tushar Sangwan is the x402 micropayment gateway for AI. One wallet, 55+ models, pay-per-request with USDC. [aloe.ai](https://aloe.ai)_
