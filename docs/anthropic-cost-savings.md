# Stop Overpaying for Claude: How ALOE Cuts Your Anthropic Bill by 70%
...
[ALOE](https://github.com/TusharSangwan/ALOE) is an open-source local proxy that sits between your app and <!-- br:models.chatVisible -->70<!-- /br:models.chatVisible --> AI models. It saves you money in three ways: **smart routing**, **token optimization**, and **response caching**.

```
┌─────────────┐     ┌──────────────────────────────┐     ┌──────────────────┐
│  Your App    │────▶│       ALOE              │────▶│  41+ AI Models   │
│  (OpenAI     │     │       (local proxy)           │     │                  │
│   SDK)       │     │                               │     │  FREE  (9 free)  │
...
ALOE scores every prompt against 15 dimensions in <1ms and routes it to the cheapest model that can handle the task.
...
Even when a request does go to Claude, ALOE reduces the tokens you pay for. The proxy runs a multi-layer compression pipeline on your request **before** sending it to the provider — and you pay based on the **compressed** token count, not the original.
...
ALOE caches responses locally. If your app sends the same request within 10 minutes, you get an instant response at **zero cost** — no API call, no tokens billed.

This is more common than you'd think:

- **Retry logic** — Your app retries on timeout. Without dedup, you pay twice. With ALOE, the retry resolves from cache instantly.
...
### ALOE (real paying-user distribution)
...
ALOE runs a weighted scoring algorithm on every prompt — entirely locally, in under 1 millisecond, zero external API calls.
...
client = OpenAI(
    ...
    api_key="unused"                       # ← ALOE handles auth
)

response = client.chat.completions.create(
    model="aloe/auto",                 # ← was: claude-sonnet-4.6
...
const response = await client.chat.completions.create({
  model: "aloe/auto", // or "eco" for max savings, "premium" for best quality
  messages: [{ role: "user", content: "Your prompt here" }],
});
```
...
- `aloe/auto` — Balanced cost/quality (default)
- `aloe/eco` — Maximum savings (free tier aggressively)
- `aloe/premium` — Best quality (Opus/Sonnet/GPT-5)
- `aloe/free` — Free tier only (gpt-oss-120b)
...
╔═══════════════════════════════════════════════════════╗
║        ALOE v0.12.12 — Usage Statistics         ║
╠═══════════════════════════════════════════════════════╣
║  Period: last 7 days                                  ║
...
The fundamental difference: **OpenRouter is a model marketplace where you choose.** ALOE is an intelligent proxy that **chooses for you**, compresses your tokens, caches your responses, and pays per-request with crypto from your own wallet.
...
- [ALOE on GitHub](https://github.com/TusharSangwan/ALOE) — MIT License
- [Tushar Sangwan](https://aloe.ai) — AI model marketplace
- [x402 Protocol](https://www.x402.org/) — Per-request crypto payments for AI

---

_Cost data based on real production traffic from paying users across 20,000+ requests, March 2026. Savings vary by workload — agent-heavy and long-context workloads see larger compression benefits. ALOE is open-source and part of the Tushar Sangwan ecosystem._
