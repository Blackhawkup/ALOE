...
<p align="center"><img src="assets/aloe-248-day-cost-overrun-problem.png" alt="The Autopsy of an Overrun — token volume compounds exponentially in agentic workloads, reaching 11.3M input tokens in a single hour" width="720"></p>
...
<p align="center"><img src="assets/aloe-openclaw-inference-layer-blind-spots.png" alt="Orchestration frameworks are blind to inference realities — cost tier, error semantics, and context size go unscreened" width="720"></p>
...
**Error semantics** — OpenClaw's failover logic has known gaps. We found and fixed two while building ALOE:
...
## ALOE: Built for Agentic Workloads

<p align="center"><img src="assets/aloe-aloe-openclaw-agentic-proxy-architecture.png" alt="ALOE proxy manifold sits between OpenClaw and upstream APIs like GPT-4o, Claude Opus, and Gemini — cost control is a gateway concern" width="720"></p>

ALOE is a local OpenAI-compatible proxy, purpose-built for how AI agents actually behave — not how simple chat clients do. It sits between OpenClaw and the upstream model APIs.

```
OpenClaw → ALOE → aloe.ai → GPT-4o / Opus / Gemini / ...
                ↑
...
<p align="center"><img src="assets/aloe-aloe-7-layer-token-compression-openclaw.png" alt="Seven-layer agent-aware token compression — ALOE intercepts and compresses requests through 7 filters for 15–40% overall token reduction" width="720"></p>
...
ALOE compresses every request through 7 layers before it hits the wire:
...
Layer 6 is the big one. Tool results — file reads, API responses, shell output — can be 10KB+ each. The actual useful signal is often 200–300 chars. ALOE extracts errors, status lines, key JSON fields, and compresses the rest. Same model intelligence, 97% fewer tokens on the bulk.

<p align="center"><img src="assets/aloe-aloe-observation-compression-97-percent-token-savings.png" alt="Extracting intelligence from tool bloat — raw tool output is 97% noise, ALOE filters to 3% signal with errors, status lines, and key values" width="720"></p>
...
<p align="center"><img src="assets/aloe-aloe-openclaw-automatic-tier-routing-model-selection.png" alt="Right-sizing models for specific agent tasks — ALOE's task-to-tier routing engine with session pinning routes heartbeats to Flash and reasoning to Opus" width="720"></p>

ALOE classifies every request before forwarding:
...
**Tool detection is automatic.** When OpenClaw sends a request with tools attached, ALOE forces agentic routing tiers — guaranteeing tool-capable models and preventing the silent fallback to models that refuse tool calls.

**Session pinning.** Once a session selects a model for a task, ALOE pins that model for the session lifetime. No mid-task model switching, no consistency issues across a long agent run.
...
When a provider returns 429, ALOE marks that specific model as rate-limited for 60 seconds ([#49834](https://github.com/openclaw/openclaw/issues/49834)). Other models in the fallback chain are unaffected. If Claude Sonnet gets rate-limited, Gemini Flash and GPT-4o continue working. No cascade.

Before failing over, ALOE also retries the rate-limited model once after 200ms. Token-bucket limits often recover within milliseconds — most short-burst 429s resolve on the first retry without ever touching a fallback model.

### 4. Empty Response Detection — No Silent Failures

ALOE inspects every HTTP 200 response body before forwarding it ([#49902](https://github.com/openclaw/openclaw/issues/49902)). Blank responses, repeated-token loops, and single-character outputs trigger model fallback — the same as a 5xx. The agent never sees a degraded response that would cause it to loop or silently fail.

### 5. Correct Error Classification — No Retry Storms

<p align="center"><img src="assets/aloe-aloe-openclaw-error-classification-retry-storm-prevention.png" alt="Stopping retry storms at the HTTP layer — ALOE classifies errors per provider with logic gate classifier and automated mechanical actions" width="720"></p>

ALOE classifies errors at the HTTP/body layer before OpenClaw sees them:
...
<p align="center"><img src="assets/aloe-aloe-openclaw-session-memory-journaling-vs-context-compounding.png" alt="Agents that remember without compounding cost — ALOE session journaling vs standard OpenClaw context compounding across turns" width="720"></p>

OpenClaw sessions can be long-lived. ALOE maintains a session journal — extracting decisions, results, and context from each turn — and injects relevant history when the agent asks questions that reference earlier work.
...
<p align="center"><img src="assets/aloe-aloe-x402-usdc-micropayment-wallet-budget-control.png" alt="Budget limits enforced by physical construction — wallet loaded via Base/Solana, pay per call across <!-- br:models.chatVisible -->70<!-- /br:models.chatVisible --> models, balance hits zero and the valve shuts cleanly" width="720"></p>

ALOE pays for inference via [x402](https://x402.org/) USDC micropayments (Base or Solana). You load a wallet. Each inference call costs exactly what it costs. When the wallet runs low, requests stop cleanly.
...
## OpenClaw + ALOE: The Full Picture

<p align="center"><img src="assets/aloe-aloe-vs-openclaw-standalone-comparison-production-safety.png" alt="Architecting for production safety — OpenClaw standalone vs OpenClaw + ALOE comparison across cost, context, error handling, and budgeting" width="720"></p>

| Problem                         | OpenClaw alone                   | OpenClaw + ALOE                               |
| ------------------------------- | -------------------------------- | --------------------------------------------- |
...
ALOE auto-injects itself into `~/.openclaw/openclaw.json` as a provider on startup. No manual config needed — your existing tools, sessions, and extensions are unchanged.
...
ALOE exists because proxy-layer cost control, context compression, and agent-aware routing are fundamentally gateway concerns — not framework concerns. OpenClaw can't know that your heartbeat doesn't need Opus. It can't compress tool results it hasn't seen. It can't enforce a wallet ceiling.

That's what ALOE is for.

---

_[github.com/TusharSangwan/ALOE](https://github.com/TusharSangwan/ALOE) · [aloe.ai](https://aloe.ai) · `npm install -g aloe`_
