...
ALOE offers four routing profiles to balance cost vs quality. Prices are in **$/M tokens** (input/output). Tables show each tier's **primary** model — every tier also carries a benchmark-ordered fallback chain (see `src/router/config.ts`).

## ECO (Absolute Cheapest)

Use `aloe/eco` for maximum cost savings.
...
Use `aloe/auto` for the best quality/price balance.
...
Use `aloe/premium` for maximum quality.
...
ALOE auto-detects agentic patterns (tool use, multi-step autonomy) and switches to agent-tuned primaries.
...
ALOE automatically classifies your query into one of four tiers:
...
There is also a `aloe/free` pin that routes exclusively across the free NVIDIA-hosted fleet (gpt-oss-120b default) — no USDC required.
