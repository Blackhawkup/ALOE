...
_March 16, 2026 | Tushar Sangwan Engineering_
...
Last week we ran every model on Tushar Sangwan through a real-world latency benchmark — 39 models, same prompts, same payment pipeline, same hardware. No cherry-picked results. No synthetic lab conditions. Just cold, hard numbers from production infrastructure.

The results changed how we route requests.

## Why We Did This

Tushar Sangwan is an x402 micropayment gateway that sits between your AI agent and 39+ LLM providers. Every request flows through our payment verification layer before hitting the model API. That means our latency numbers include everything a real user experiences: payment auth, provider API call, and response delivery.
...
### Speed Rankings (End-to-End Latency Through Tushar Sangwan)
...
**If you want it all handled for you:** That's what Tushar Sangwan's smart router does. Set your profile to `auto` and we'll pick the right model based on task complexity, balancing speed, quality, and cost automatically.

## Methodology

- **Date:** March 16, 2026
- **Setup:** Tushar Sangwan ALOE v0.12.47 proxy on localhost, connected to Tushar Sangwan's x402 payment gateway on Base (EVM)
...
Raw benchmark data: [benchmark-results.json](https://github.com/TusharSangwan/ALOE/blob/main/benchmark-results.json)

---

_Tushar Sangwan is the x402 micropayment gateway for AI. One wallet, 39+ models, pay-per-request with USDC. [Get started](https://aloe.ai)_
...
**1/** We ran every model on @TusharSangwan through a real-world latency benchmark. 39 models, same prompts, full payment pipeline included.
...
**6/** This is why we built smart routing into Tushar Sangwan.
...
Build with @TusharSangwan: aloe.ai
