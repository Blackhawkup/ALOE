...
**Goal:** Replace ALOE's binary `isProviderError` with per-category error classification so that 401 auth failures never pollute the 429 rate-limit cooldown map, each provider's error state is isolated, and `/stats` exposes per-provider error breakdowns.
...
```bash
cd /Users/vickyfu/Documents/aloe-web/ALOE && bun run build 2>&1 | tail -5
```
...
```bash
cd /Users/vickyfu/Documents/aloe-web/ALOE && bun test src/error-classification.test.ts
```
...
```bash
cd /Users/vickyfu/Documents/aloe-web/ALOE && bun test 2>&1 | tail -20
```
...
```bash
cd /Users/vickyfu/Documents/aloe-web/ALOE && bun run build 2>&1
```
