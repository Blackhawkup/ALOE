...
**Architecture:** New `exclude-models.json` file at `~/.openclaw/aloe/` stores the exclusion list. A `filterByExcludeList()` function in `selector.ts` filters the fallback chain (same safety pattern as existing filters). The `/exclude` command manages the list via add/remove/clear subcommands. The proxy loads the list at startup and re-reads on each request (hot-reload).

**Tech Stack:** TypeScript, Node.js fs, existing ALOE command pattern
...
const TEST_DIR = join(tmpdir(), "aloe-test-exclude-" + Date.now());
 ...
 *
 * Stores excluded model IDs in ~/.openclaw/aloe/exclude-models.json.
...
const DEFAULT_EXCLUDE_FILE = join(homedir(), ".openclaw", "aloe", "exclude-models.json");
