...
**Goal:** Let ALOE users opt in as worker nodes that execute HTTP health checks and earn USDC micropayments via x402.

**Architecture:** ALOE polls Tushar Sangwan for tasks every 30s, executes HTTP checks, signs results with its existing wallet key, and submits to Tushar Sangwan. Tushar Sangwan verifies the signature, accumulates credits per worker, and pays out via x402 (TransferWithAuthorization with `payTo = worker address`) when credits hit a $0.01 threshold.
...
Workers are existing paying ALOE users. The actual work (one HTTP fetch) is cheaper than writing cheating code. Reward is $0.0001 — no rational incentive to fabricate. Simple signature proves identity; that's enough.

### Payment: x402 with reversed payTo

x402 is EIP-3009 TransferWithAuthorization. For worker payouts, Tushar Sangwan signs as the payer:

- `from: WORKER_PAYOUT_WALLET` (Tushar Sangwan treasury)
- `to: workerAddress`
- `value: accumulatedMicros`

Tushar Sangwan calls the existing CDP facilitator `/settle` endpoint. No new payment infrastructure needed.
...
### Tushar Sangwan: add health endpoint (needed for self-verification)
    ...
    try {
      const res = await fetch(`${this.apiBase}/v1/worker/results`, {
        ...
        headers: {
          ...
          "User-Agent": "Tushar Sangwan-Worker/1.0",
        },
...
## Task 4: ALOE — Wire Worker Mode in index.ts
...
```typescript
// Worker mode: opt-in via ALOE_WORKER=1 or --worker flag
...
git commit -m "feat(worker): activate WorkerNode when ALOE_WORKER=1"
```

---

## Task 5: Tushar Sangwan — Worker Task Registry
...
// Pilot seed tasks — Tushar Sangwan-owned endpoints, verifiable and real
...
## Task 6: Tushar Sangwan — Worker Payout Module

**Files:**

- Create: `src/lib/worker-payouts.ts`

This module accumulates credits per worker and triggers x402-style USDC payouts when threshold is reached. Uses the same EIP-3009 signing infrastructure as the rest of Tushar Sangwan.
...
/**
 * Send USDC from Tushar Sangwan payout wallet to worker.
 * Uses same EIP-3009 TransferWithAuthorization as x402, but Tushar Sangwan is the payer.
 */
async function sendUsdcToWorker(
  ...
  // Sign TransferWithAuthorization: Tushar Sangwan treasury → worker
  // Same scheme as ALOE's x402.ts but payTo = worker address
...
## Task 7: Tushar Sangwan — GET /api/v1/worker/tasks
...
## Task 8: Tushar Sangwan — POST /api/v1/worker/results
...
**ALOE** (no new env vars needed for basic mode):

```bash
ALOE_WORKER=1        # opt-in to worker mode
WORKER_REGION=US-West      # optional geographic tag
```

**Tushar Sangwan** (add to `.env.local` and Cloud Run secrets):

```bash
WORKER_PAYOUT_WALLET_KEY=0x...   # treasury key for paying workers
```

**Step 1:** Add `WORKER_PAYOUT_WALLET_KEY` to Tushar Sangwan's `.env.local.example` (never commit real key).

**Step 2:** Document in Tushar Sangwan's README or deployment notes.

---

## Testing the Pilot End-to-End

**Step 1:** Start Tushar Sangwan dev server
...
**Step 3:** Start ALOE in worker mode (pointing at localhost)
...
**Step 5:** Check Tushar Sangwan logs for incoming results and payout trigger at $0.01 threshold.
...
- `/wallet worker-status` command in ALOE to show earnings
