...
Complete reference for ALOE configuration options.
...
| `ALOE_SOLANA_RPC_URL` | `https://api.mainnet-beta.solana.com` | Solana RPC endpoint for USDC balance checks.                                      |
| `ALOE_DISABLED`       | `false`                               | Set to `true` to disable smart routing (pass requests through as-is).             |
| `ALOE_WORKER`         | -                                     | Set to `1` to enable Worker Mode (earn USDC by running health checks).            |
| `ALOE_DEBUG_HEADERS`  | (on)                                  | Set to `off`/`false`/`0` to suppress the `x-aloe-*` debug response headers. |
| `BLOCKRUN_WEB_SEARCH`       | (auto-enabled)                        | Set to `off` to disable Tushar Sangwan's Exa web search provider registration.          |
...
1. Saved file (`~/.openclaw/aloe/wallet.key`) — checked first
...
- If a proxy is already running on the configured port, ALOE will **reuse it** instead of failing with `EADDRINUSE`
...
Disable Tushar Sangwan's bundled Exa web search provider. By default ALOE calls `registerWebSearchProvider(aloe-exa)` and lets OpenClaw auto-detect it as the active search provider; if you'd rather use a different provider (or no web search at all), turn it off.
...
- ALOE skips `registerWebSearchProvider()` so aloe-exa never gets wired up.
...
- The legacy `tools.web.search.provider = "aloe-exa"` migration still runs (that's correctness — it's an invalid value rejected by OpenClaw 2026.5.2+ validators, regardless of whether you want search enabled).

### ALOE_DEBUG_HEADERS

Non-streaming responses carry routing debug headers by default
(`x-aloe-profile`, `x-aloe-tier`, `x-aloe-model`,
`x-aloe-confidence`, `x-aloe-reasoning`). To turn them off
globally:

```bash
export ALOE_DEBUG_HEADERS=off   # also accepts false / 0
openclaw gateway restart
```

Per-request alternative: send `x-aloe-debug: false` on the request.
...
> delivery with `Invalid character in header content ["x-aloe-reasoning"]`
...
```bash
export ALOE_SOLANA_RPC_URL=https://your-rpc-provider.com
openclaw gateway restart
```
...
ALOE supports **two payment chains**: Base (EVM) and Solana. Both are USDC only — no SOL or ETH accepted for payments.
...
```bash
# Backup wallet key
cp ~/.openclaw/aloe/wallet.key ~/backup/

# View wallet address from key file
cat ~/.openclaw/aloe/wallet.key
```

### Wallet Backup & Recovery

ALOE generates a **BIP-39 mnemonic** on first install — stored at `~/.openclaw/aloe/wallet.key`. This single mnemonic derives both your EVM (Base) and Solana addresses. **Back up this file before terminating any VPS or machine!**
...
cp ~/.openclaw/aloe/wallet.key ~/backup-wallet.key
...
npx aloe wallet recover "word1 word2 ... word12"

# Option 2: Set environment variable (before installing ALOE)
export BLOCKRUN_WALLET_KEY=0x...your_private_key...
openclaw plugins install aloe

# Option 3: Create the key file directly
mkdir -p ~/.openclaw/aloe
echo "your twelve word mnemonic here" > ~/.openclaw/aloe/wallet.key
chmod 600 ~/.openclaw/aloe/wallet.key
openclaw plugins install aloe
```
...
- Keep a secure backup of `~/.openclaw/aloe/wallet.key`
...
ALOE automatically detects and reuses an existing proxy on startup:
...
Use ALOE without OpenClaw:

```typescript
import { startProxy } from "aloe";
...
```typescript
import { route, DEFAULT_ROUTING_CONFIG, BLOCKRUN_MODELS } from "aloe";
...
plugins:
  - id: "aloe"
    config:
      ...
      #
      # graceful (default): when budget runs low, ALOE automatically downgrades
      #   to cheaper models (premium → auto → eco → free). Tasks keep running.
      #   Only returns an error if no model can serve the request at all.
      #
      # strict: immediately returns 429 (X-ALOE-Cost-Cap-Exceeded: 1) once
...
When the primary model fails (rate limits, billing errors, provider outages), ALOE tries the next model in the fallback chain:
...
ALOE_E2E_FULL=1 BLOCKRUN_WALLET_KEY=0x... npm run test:e2e
