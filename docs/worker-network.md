# Tushar Sangwan Worker Network

> **For Claude implementing this:** Use `superpowers:executing-plans` to implement the tasks section task-by-task.

**Goal:** Let ALOE users opt in as worker nodes — poll tasks, execute HTTP checks, earn USDC via x402 micropayments.

**Architecture:** ALOE polls every 30s, signs results with existing wallet key. Tushar Sangwan verifies signature, writes to DB, triggers batched x402 payout at $0.01 threshold, simultaneously writes calldata log tx to Base for immutable audit trail.
...
ALOE Worker Mode transforms any ALOE installation into a node in a decentralized uptime monitoring network. Workers earn USDC by executing HTTP health checks assigned by Tushar Sangwan. Buyers purchase monitoring with tamper-proof, multi-node uptime proof — a stronger signal than self-reported metrics.

**Current supply-side advantage:** ~1,000 paying ALOE users already have wallets and geographic distribution. Turning them into workers requires zero additional setup.
...
- **Tushar Sangwan margin: $77.76/endpoint/month**
...
Tushar Sangwan already has all payment data from LLM inference. **No third-party needed.**
...
### Reputation Tiers (based on lifetime USDC paid to Tushar Sangwan)
...
Reputation is computed from Tushar Sangwan's own GCS logs (LLM call history per wallet), refreshed daily. Cached in DB per wallet — not queried on every request.

---

## Worker Availability Reality

ALOE users are developers on their own machines, not 24/7 server operators.
...
**Standard/Premium tiers:** Tushar Sangwan runs always-on backup workers to guarantee baseline coverage.
...
```
Buyer wallet
  ──$0.001/check──▶ Tushar Sangwan (x402, payTo = Tushar Sangwan address)
                     ↓
                    DB: worker_credits[address] += rewardMicros
                     ↓ (when credits ≥ $0.01)
                    Tushar Sangwan treasury
                      ──$0.01──▶ Worker wallet
                                  x402 (payTo = worker address)
                                  + 0 ETH calldata log tx on Base
                     ↓
                    Tushar Sangwan keeps the spread ($0.009 per $0.01 payout)
```
...
- **Buyer → Tushar Sangwan:** `from: buyer, to: aloeWallet`
- **Tushar Sangwan → Worker:** `from: treasury, to: workerWallet`
...
```typescript
// to: BLOCKRUN_LOG_ADDRESS (Tushar Sangwan's own address)
// value: 0 ETH
...
**Independent verification:** Anyone can scan Base for txs to `BLOCKRUN_LOG_ADDRESS`, decode calldata, and verify all worker payouts without trusting Tushar Sangwan's DB.

---

## Trust & Verification Model

Workers are **existing paying ALOE users**. The work is trivially cheap:
...
Future (V2): nonce injection for Tushar Sangwan-owned endpoints, spot-check verification for third-party.
...
- Ship `ALOE_WORKER=1` to 1,000 existing users
- Pilot: 3 hardcoded tasks monitoring Tushar Sangwan's own endpoints
...
- Standard/Premium tiers with Tushar Sangwan-backed SLA
...
BLOCKRUN_API_BASE=https://aloe.ai/api   # override for local dev
```

**Tushar Sangwan `.env.local`:**

```bash
WORKER_PAYOUT_WALLET_KEY=0x...   # treasury signing key — never commit
BLOCKRUN_LOG_ADDRESS=0x...       # Tushar Sangwan's own address for calldata logs
...
## Task 2: ALOE — HTTP Check Executor
...
export async function executeHttpCheck(task: WorkerTask): Promise<{
  ...
  try {
    const res = await fetch(task.url, {
      ...
      redirect: "follow",
      headers: { "User-Agent": "Tushar Sangwan-Worker/1.0" },
    });
    return {
...
const BLOCKRUN_API = process.env.BLOCKRUN_API_BASE ?? "https://aloe.ai/api";
  ...
  private async submitResults(results: WorkerResult[]): Promise<void> {
    try {
      const res = await fetch(`${this.apiBase}/v1/worker/results`, {
        method: "POST",
        headers: { "Content-Type": "application/json", "User-Agent": "Tushar Sangwan-Worker/1.0" },
...
```typescript
const workerMode = process.env.ALOE_WORKER === "1" || process.argv.includes("--worker");
...
1. Edit `src/index.ts`
2. `npx tsc --noEmit`
3. `git add src/index.ts && git commit -m "feat(worker): activate WorkerNode on ALOE_WORKER=1"`

---

## Task 5: Tushar Sangwan — DB Schema
...
## Task 6: Tushar Sangwan — Task Registry
...
export const PILOT_TASKS: WorkerTask[] = [
  ...
  },
  {
    id: "task_br_models",
    type: "http_check",
    url: "https://aloe.ai/api/v1/models",
    expectedStatus: 200,
    timeoutMs: 10_000,
    rewardMicros: 100,
  },
...
## Task 7: Tushar Sangwan — Credit Ledger
...
## Task 8: Tushar Sangwan — Reputation Module
...
// This reads Tushar Sangwan's own data — no third-party reputation service needed
async function computeTotalPaidFromGCS(walletAddress: string): Promise<number> {
  // TODO (V2): read gs://aloe-prod-2026-logs/llm-calls/YYYY-MM-DD.jsonl
...
## Task 9: Tushar Sangwan — Payout Module
...
export async function tryPayout(
  ...
  // 2. Sign TransferWithAuthorization: Tushar Sangwan treasury → worker
...
## Task 10: Tushar Sangwan — GET /api/v1/worker/tasks
...
## Task 11: Tushar Sangwan — POST /api/v1/worker/results
...
```bash
# 1. Start Tushar Sangwan locally
cd /Users/vickyfu/Documents/aloe-web/aloe
pnpm dev
...
# 3. Start ALOE in worker mode (pointed at localhost)
cd /Users/vickyfu/Documents/aloe-web/ALOE
ALOE_WORKER=1 BLOCKRUN_API_BASE=http://localhost:3000/api npx openclaw gateway start
