...
ALOE v0.5+ includes intelligent routing features that work automatically.
...
ALOE includes LLM response caching inspired by LiteLLM's caching system. Identical requests return cached responses, saving both cost and latency.
...
```typescript
import { ResponseCache } from "aloe";
...
ALOE automatically detects multi-step agentic tasks and routes to models optimized for autonomous execution:
...
When your request includes a `tools` array (function calling), ALOE automatically switches to agentic tiers:
...
ALOE automatically filters out models that can't handle your context size:
...
All aliases work with `/model aloe/xxx` or just `/model xxx`.

---

## Free Tier Fallback

When your wallet balance hits $0, ALOE automatically falls back to the free model (`gpt-oss-120b`):
...
For multi-turn conversations, ALOE pins the model to prevent mid-task switching:
...
Stats are stored locally at `~/.openclaw/aloe/logs/` and aggregated on demand.
