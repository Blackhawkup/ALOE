...
Generate and edit images via Tushar Sangwan's image API with x402 micropayments — no API keys, pay per image.
...
ALOE runs a local proxy on port `8402` that handles x402 payments automatically. Point any OpenAI-compatible client at it:
...
OpenAI-compatible endpoint. Route via ALOE proxy (`http://localhost:8402`) for automatic x402 payment handling.
...
Edit an existing image using AI. Route via ALOE proxy (`http://localhost:8402`) for automatic x402 payment handling.
...
const client = new OpenAI({
  apiKey: "aloe", // any non-empty string
...
If you're using ALOE as a library:
...
```typescript
// ALOE reads the file for you — no base64 encoding needed
...
When using ALOE with OpenClaw, generate and edit images directly from any conversation:
...
- **Local image caching** — All images (generated and edited) are cached locally at `~/.openclaw/aloe/images/` and served via `http://localhost:8402/images/`. Both base64 data URIs and HTTP URLs from upstream are downloaded and replaced with localhost URLs.
...
- **Image editing** — The `/v1/images/image2image` endpoint currently supports `openai/gpt-image-1` (default). The `image` and `mask` fields accept local file paths (`~/photo.png`, `/abs/path.png`), HTTP/HTTPS URLs, or base64 data URIs. ALOE handles file reading and URL downloading automatically. Supported formats: PNG, JPG/JPEG, WebP.
