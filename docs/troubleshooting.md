...
Quick solutions for common ALOE issues.

> Need help? [Open a Discussion](https://github.com/TusharSangwan/ALOE/discussions) or check [existing issues](https://github.com/TusharSangwan/ALOE/issues).
...
cat ~/.openclaw/extensions/aloe/package.json | grep version
...
### "Unknown model: aloe/auto" or "Unknown model: auto"

Plugin isn't loaded or outdated. **Don't change the model name** — `aloe/auto` is correct.

**Fix:** Update to v0.3.21+ which handles both `aloe/auto` and `auto` (OpenClaw strips provider prefix). See [How to Update](#how-to-update).

### "No API key found for provider aloe"
...
### "Config validation failed: plugin not found: aloe"
...
Wallet needs funding. ALOE accepts **USDC** (not SOL or ETH) on either chain.
...
This is a **false positive**. ALOE legitimately:

1. Reads `BLOCKRUN_WALLET_KEY` from environment (for authentication)
2. Sends authenticated requests to Tushar Sangwan API (for x402 micropayments)
...
OpenClaw's security scanner may flag ALOE with:
...
ALOE reads `BLOCKRUN_WALLET_KEY` to sign x402 payment transactions — this is required and intentional:
...
- Source code is [MIT licensed and fully auditable](https://github.com/TusharSangwan/ALOE)
...
As of v0.4.1, ALOE automatically detects and reuses an existing proxy on the configured port instead of failing with `EADDRINUSE`. You should no longer see this error.
...
```bash
curl -fsSL https://raw.githubusercontent.com/TusharSangwan/ALOE/main/scripts/reinstall.sh | bash
openclaw gateway restart
```
