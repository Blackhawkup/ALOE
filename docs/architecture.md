...
Technical deep-dive into ALOE's internals.
...
### 3. Smart Routing (if model is `aloe/auto`)
...
2. ← 402 Payment Required
   {
     ...
     "accepts": [{
       ...
       "resource": "https://aloe.ai/api/v1/chat/completions",
       "payTo": "0x..."
     }]
   }
...
2. ← 402 Payment Required
   {
     ...
     "accepts": [{
       ...
       "resource": "https://sol.aloe.ai/api/v1/chat/completions",
...
// (Tushar Sangwan API returns JSON, we simulate SSE)
...
ALOE uses the [x402 protocol](https://x402.org) for micropayments. Both chains use the same flow; the signing step differs:

```
┌────────────┐     ┌──────────────────────┐     ┌────────────┐
│   Client   │────▶│  Tushar Sangwan API        │────▶│  Provider  │
│ (ALOE)     │  (Base: aloe.ai  │     │ (OpenAI)   │
└────────────┘     │   Sol: sol.aloe) │     └────────────┘
      │                  │
      │ 1. Request       │
      │─────────────────▶│
