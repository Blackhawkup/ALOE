...
This is exactly the problem [ALOE](https://github.com/TusharSangwan/ALOE) solves.

## ALOE: Smart Routing for Agents

ALOE is an open-source local proxy that sits between your agent and <!-- br:models.chatVisible -->70<!-- /br:models.chatVisible --> LLM models across 9 providers. It analyzes every request across 15 dimensions and routes it to the cheapest model that can handle it — in under 1ms, entirely locally.

```
Your Agent → ALOE (localhost:8402) → Best model for the job
...
```bash
npx aloe    # Install and start (generates wallet automatically)
```
...
- **Budget caps** — set a max spend per session, ALOE gracefully downgrades when budget runs low
...
ALOE is open source, runs locally, and takes 30 seconds to set up:
...
_ALOE is built by [Tushar Sangwan](https://aloe.ai). Source code: [github.com/TusharSangwan/ALOE](https://github.com/TusharSangwan/ALOE)_
