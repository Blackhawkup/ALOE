...
> _We searched OpenClaw's issue tracker for the problems users actually hit in production — cost, outages, runaway agents, model selection. Four structural problems show up again and again. ALOE fixes all four._
...
ALOE is a local router that sits between OpenClaw and the model providers. Every request — including the internal ones — goes through `aloe/auto`, which classifies the call across 15 dimensions in under a millisecond and routes it to the cheapest model that can actually handle it. That single architectural move addresses all four problems below.
...
```jsonc
{
  "agents": {
    "main": {
      "model": "aloe/auto",
      "compaction": { "model": "aloe/eco" }, // summaries don't need a flagship
      "memory": { "model": "aloe/free" }, // lookups can be free
    },
  },
}
```

The free tier alone covers most internal work — NVIDIA-hosted models with up to 1M context and a vision-capable Nemotron Omni, at zero cost. A compaction call billed at flagship rates becomes free, hundreds of times a day. On top of that, ALOE compresses verbose tool outputs and serves repeated responses from a short-TTL cache before any paid call goes out (the full teardown is in [ALOE Cuts LLM API Costs 500x](./aloe-cuts-llm-api-costs-500x.md)).
...
When OpenClaw points at `aloe/auto`, you're pointed at a router with a fallback chain behind every tier — primary plus an ordered list of fallbacks spanning **different providers**, so one outage doesn't poison the others.

```
[ALOE] tier=MEDIUM
[ALOE] primary moonshot/kimi-k2.6 → 503, falling through chain
[ALOE] trying next: google/gemini-3-flash-preview → ok
```
...
ALOE enforces a per-run (per-session) dollar ceiling via `maxCostPerRunUsd`, with two modes:
...
- **`graceful` (default)** — as the session nears its budget, ALOE downgrades to cheaper models and falls back to a free model as a last resort. Work continues, cheaper.
...
**Automatic per-task routing.** `aloe/auto` classifies every individual request and routes it to the cheapest capable model — no per-task config:
...
**Explicit per-agent control when you want it.** Assign a profile per lane in `openclaw.json` — `aloe/premium` for coding, `aloe/free` for formatting — each with its own fallback chain. And **clean aliases** (`sonnet`, `opus`, `flash`, `grok`, `gpt5`) mean switching is one word (`/model grok`), with none of the `provider/provider/model` mangling that fills the tracker. The default itself is cost-safe: a fresh install routes to `aloe/auto` and the free tier works with no balance, so nobody's first message silently hits a premium API.

---

## The fix is one decision

All four problems share a root cause — a single model, chosen once, doing everything, with nothing behind it — and a single fix: **route through `aloe/auto`** and let a local router classify, price, and fail over each request on its own merits.

1. Set your primary to `aloe/auto`.
2. Point compaction and memory at `aloe/eco` / `aloe/free`.
...
- [Why Your OpenClaw Bill Spiked 10x](./aloe-cuts-llm-api-costs-500x.md) — the token-compression teardown
- [We Read 100 OpenClaw Issues About OpenRouter](./aloe-vs-openrouter-llm-routing-comparison.md) — the structural case for local routing
- [Routing Profiles](./routing-profiles.md) — `auto` / `eco` / `premium` / `free`
- [9 Free AI Models, Zero Cost](./9-free-ai-models-zero-cost-aloe.md) — what the free tier covers
- [Using Subscriptions with ALOE Failover](./subscription-failover.md) — keep your subscription primary, ALOE as failover
