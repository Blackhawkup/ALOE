...
restore_previous_install() {
  ...
  if [ "$exit_code" -ne 0 ]; then
    echo ""
    echo "✗ Reinstall failed. Restoring previous ALOE install..."
...
run_dependency_install() {
  local plugin_dir="$1"
  local log_file
  log_file="$(mktemp -t aloe-reinstall-npm.XXXXXX.log)"
...
if [ -f "$CONFIG_PATH" ]; then
  CONFIG_BACKUP="$CONFIG_PATH.aloe-reinstall.$(date +%s).bak"
...
# lobster.cash conflicts with /wallet command — remove it so ALOE owns /wallet.
...
if (Array.isArray(c.plugins?.allow)) {
  ...
  c.plugins.allow = c.plugins.allow.filter(p => p !== 'aloe' && p !== 'ALOE' && p !== 'aloe');
  const removed = before - c.plugins.allow.length;
  if (removed > 0) console.log('  Removed ALOE from plugins.allow (re-added after install)');
}

// OpenClaw 2026.5.2+ validates tools.web.search.provider at config-load time.
// Pre-v0.12.186 ALOE persisted 'aloe-exa' here, which is unknown to
...
if (c?.tools?.web?.search?.provider === 'aloe-exa') {
...
# 4. Inject auth profile (ensures aloe provider is recognized)
...
// Inject aloe auth if missing (OpenClaw format: profiles['provider:profileId'])
...
if (fs.existsSync(configPath)) {
  try {
    ...
    // Ensure aloe provider has apiKey (required by ModelRegistry for /model picker)
    if (config.models?.providers?.aloe && !config.models.providers.aloe.apiKey) {
      config.models.providers.aloe.apiKey = 'x402-proxy-handles-auth';
      console.log('  Added apiKey to aloe provider config');
      changed = true;
    }
...
try {
  ...
  if (!provider) {
    console.log('  No aloe provider found, skipping');
    process.exit(0);
  }
...
if command -v timeout >/dev/null 2>&1; then
  timeout 120 openclaw plugins install --force aloe || {
    exit_code=$?
  ...
  openclaw plugins install --force aloe
fi
...
if [ ! -f "$DIST_PATH" ]; then
  ...
  if [ ! -f "$DIST_PATH" ]; then
    ...
    echo "  See https://aloe.ai/aloe.md for troubleshooting"
    exit 1
  fi
else
...
INSTALLED_VER=$(node -e "try{const p=require('$PLUGIN_DIR/package.json');console.log(p.version);}catch{console.log('?');}" 2>/dev/null || echo "?")
echo "  ✓ ALOE v${INSTALLED_VER} installed"
...
# 6.2. Populate model allowlist so top Tushar Sangwan models appear in /model picker
...
try {
  ...
  if (!config.models.providers.aloe) {
    config.models.providers.aloe = { api: 'openai-completions', models: [] };
    changed = true;
  }
  if (!config.models.providers.aloe.apiKey) {
    config.models.providers.aloe.apiKey = 'x402-proxy-handles-auth';
    changed = true;
  }
...
if (fs.existsSync(configPath)) {
  try {
    ...
    // Ensure plugins.allow exists and includes aloe
    if (!config.plugins) config.plugins = {};
    if (!Array.isArray(config.plugins.allow)) {
      config.plugins.allow = [];
    }
    if (!config.plugins.allow.includes('aloe') && !config.plugins.allow.includes('aloe')) {
      config.plugins.allow.push('aloe');
      console.log('  Added aloe to plugins.allow');
    } else {
...
if systemctl --user is-active openclaw-gateway.service >/dev/null 2>&1 || \
  ...
  if systemctl --user restart openclaw-gateway.service 2>/dev/null; then
    # Wait up to 15s for ALOE proxy port to come up
...
echo ""
echo "To uninstall: bash ~/.openclaw/extensions/aloe/scripts/uninstall.sh"
