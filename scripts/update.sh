...
PLUGIN_DIR="$HOME/.openclaw/extensions/aloe"
CONFIG_PATH="$HOME/.openclaw/openclaw.json"
WALLET_FILE="$HOME/.openclaw/aloe/wallet.key"
WALLET_BACKUP=""
PLUGIN_BACKUP=""
CONFIG_BACKUP=""
ACTIVE_PACKAGE_DIR=""
...
const candidates = [
  { packagePath: path.join(home, '.openclaw', 'extensions', 'aloe', 'package.json'), priority: 0 },
  { packagePath: path.join(home, '.openclaw', 'npm', 'node_modules', '@aloe', 'aloe', 'package.json'), priority: 2 },
];
...
try {
  for (const name of fs.readdirSync(projectsDir)) {
    candidates.push({
      packagePath: path.join(projectsDir, name, 'node_modules', '@aloe', 'aloe', 'package.json'),
      priority: 1,
    });
  }
} catch {}
...
sync_cli_shim() {
  ...
  if [ -z "$package_dir" ] || [ ! -f "$package_dir/dist/cli.js" ]; then
    echo "  ⚠ Could not update CLI shim: ALOE CLI not found"
    return 1
  fi
  ...
  mkdir -p "$bin_dir"
  rm -f "$bin_dir/aloe"
  cat >"$bin_dir/aloe" <<EOF
#!/bin/sh
exec node "$package_dir/dist/cli.js" "\$@"
EOF
  chmod +x "$bin_dir/aloe"
  echo "  ✓ CLI shim updated: $bin_dir/aloe → $package_dir/dist/cli.js"
}
...
restore_previous_install() {
  ...
  if [ "$exit_code" -ne 0 ]; then
    echo ""
    echo "✗ Update failed. Restoring previous ALOE install..."
...
run_dependency_install() {
  local plugin_dir="$1"
  local log_file="$HOME/aloe-npm-install.log"
...
validate_config
ACTIVE_PACKAGE_DIR=$(find_aloe_package_dir)
...
if [ -f "$WALLET_FILE" ]; then
  ...
  if [[ "$WALLET_KEY" == 0x* ]] && [ "$KEY_LEN" -eq 66 ]; then
    ...
    WALLET_BACKUP="$HOME/.openclaw/aloe/wallet.key.bak.$(date +%s)"
...
if [ -d "$PLUGIN_DIR" ]; then
  PLUGIN_BACKUP="$HOME/.openclaw/aloe/aloe.backup.$(date +%s)"
...
if [ -f "$CONFIG_PATH" ]; then
  CONFIG_BACKUP="$CONFIG_PATH.aloe-update.$(date +%s).bak"
...
# Remove it so ALOE owns /wallet without conflict.
...
try {
  ...
  // Clean plugins.allow — remove only ALOE entries; preserve every other
  // plugin the user has allowed, including bare local/custom plugin IDs.
  if (Array.isArray(config?.plugins?.allow)) {
    const before = config.plugins.allow.length;
    config.plugins.allow = config.plugins.allow.filter(p => p !== 'aloe' && p !== 'ALOE' && p !== 'aloe');
    const removed = before - config.plugins.allow.length;
    if (removed > 0) { changed = true; console.log('  Removed ALOE from plugins.allow (re-added after install)'); }
  }

  // OpenClaw 2026.5.2+ validates tools.web.search.provider at config-load
  // time. Pre-v0.12.186 ALOE persisted 'aloe-exa' here, which is
  ...
  if (config?.tools?.web?.search?.provider === 'aloe-exa') {
    delete config.tools.web.search.provider;
    changed = true;
    console.log('  Removed legacy tools.web.search.provider=aloe-exa (set at runtime now)');
  }
...
try {
  const config = JSON.parse(fs.readFileSync(configPath, 'utf8'));
  const provider = config?.models?.providers?.aloe;
  if (!provider) { console.log('  No aloe provider, skipping'); process.exit(0); }
  let changed = false;
...
for stale in "$HOME/.openclaw/extensions/aloe.backup."* "$HOME/.openclaw/extensions/.openclaw-install-stage-"*; do
  [ -d "$stale" ] && rm -rf "$stale"
done
...
function getBlockrunModels(config) {
  return config?.models?.providers?.aloe?.models;
}
...
if (!Array.isArray(activeModels) || !Array.isArray(rejectedModels)) {
  fail('aloe model list is missing or invalid');
}
...
activeWithoutModelList.providers.aloe.models = [];
rejectedWithoutModelList.providers.aloe.models = [];
...
if (residualModelDelta > 4096) {
  fail('models section changed beyond the aloe model list');
}
...
if (totalDrop <= 0 || modelListDrop / totalDrop < 0.65) {
  fail('size drop is not primarily from the aloe model list');
}

active.models.providers.aloe.models = rejectedModels;
...
console.log(
  `  ✓ Applied scoped Tushar Sangwan model-list trim (${activeModels.length} -> ${rejectedModels.length})`,
);
NODE
}
...
# install ALOE files directly from npm below.
...
# "Installed plugin: aloe" in OpenClaw v2026.4.5 (parallel plugin loading).
# 120s is enough for slow connections; the install itself completes in ~30s.
if command -v timeout >/dev/null 2>&1; then
  timeout 120 openclaw plugins install --force aloe 2>&1 | tee "$OPENCLAW_INSTALL_LOG" || {
    exit_code=$?
  ...
  openclaw plugins install --force aloe 2>&1 | tee "$OPENCLAW_INSTALL_LOG" || {
    exit_code=$?
    handle_openclaw_install_failure "$exit_code" || {
      [ "$OPENCLAW_INSTALL_RECOVERABLE" = "1" ] || exit $exit_code
    }
...
force_install_from_npm() {
  ...
  local TMPPACK
  TMPPACK=$(mktemp -d)
  if npm pack "aloe@${version}" --pack-destination "$TMPPACK" --prefer-online >/dev/null 2>&1; then
    local TARBALL
    TARBALL=$(ls "$TMPPACK"/aloe-aloe-*.tgz 2>/dev/null | head -1)
    if [ -n "$TARBALL" ]; then
      rm -rf "$PLUGIN_DIR"
      mkdir -p "$PLUGIN_DIR"
...
LATEST_VER=$(npm view aloe@latest version 2>/dev/null || echo "")
if [ -z "$LATEST_VER" ]; then
  echo "  ✗ Could not resolve latest ALOE version from npm"
  exit 1
fi
...
if [ -z "$ACTIVE_PACKAGE_DIR" ] || [ -z "$INSTALLED_VER" ]; then
  echo "  ✗ Could not verify ALOE install"
  exit 1
fi

echo "  ✓ ALOE v${INSTALLED_VER} installed at $ACTIVE_PACKAGE_DIR"
...
# openclaw plugins install writes plugins.entries.ALOE (PascalCase, from
# plugin name) AND plugins.installs.aloe (lowercase, from plugin id).
...
if (!store.profiles[profileKey]) {
  store.profiles[profileKey] = { type: 'api_key', provider: 'aloe', key: 'x402-proxy-handles-auth' };
...
try {
  ...
  const currentKeys = new Set(TOP_MODELS.map(id => 'aloe/' + id));

  // Remove any aloe/* entries not in the current TOP_MODELS list
  let removed = 0;
  for (const key of Object.keys(allowlist)) {
    if (key.startsWith('aloe/') && !currentKeys.has(key)) {
...
# plugin detection), new ones live in aloe/. Clean both locations.
echo "→ Cleaning up stale plugin backups..."
CLEANED=0
for backup_dir in "$HOME/.openclaw/extensions/aloe.backup."* "$HOME/.openclaw/aloe/aloe.backup."*; do
...
try {
  const config = JSON.parse(fs.readFileSync(configPath, 'utf8'));
  let changed = false;
  const isStale = (p) => p.includes('.openclaw-install-stage-') || p.includes('aloe.backup.');
...
# If the old JSON index still contains a stale ALOE record, OpenClaw keeps
# warning about conflicting install metadata on every doctor/restart. Clean only
# ALOE's legacy record after this updater has verified the current install.
echo "→ Cleaning stale ALOE install metadata..."
node -e "
const fs = require('fs');
const os = require('os');
const path = require('path');
...
function currentPackageCandidates() {
  const candidates = [
    { packagePath: path.join(home, '.openclaw', 'extensions', 'aloe', 'package.json'), priority: 0 },
    {
      packagePath: path.join(home, '.openclaw', 'npm', 'node_modules', '@aloe', 'aloe', 'package.json'),
      priority: 2,
    },
  ];
  const projectsDir = path.join(home, '.openclaw', 'npm', 'projects');
...
try {
  ...
  if (!current) {
    console.log('  Skipped: current ALOE package.json not found');
    process.exit(0);
  }

  const index = JSON.parse(fs.readFileSync(legacyPath, 'utf8'));
  const currentPkg = current.pkg;
  const legacyRecord = index?.installRecords?.aloe;

  if (!legacyRecord) {
    console.log('  ✓ Legacy install index has no ALOE record');
    process.exit(0);
  }
  ...
  if (!stale) {
    console.log('  ✓ Legacy ALOE install metadata already matches current install');
    process.exit(0);
  }

  delete index.installRecords.aloe;
  if (Array.isArray(index.plugins)) {
    index.plugins = index.plugins.filter((plugin) => plugin?.pluginId !== 'aloe');
  }
  index.refreshReason = 'aloe-stale-metadata-cleanup';
  index.generatedAtMs = Date.now();
  atomicWrite(legacyPath, JSON.stringify(index, null, 2));
  console.log('  ✓ Removed stale legacy ALOE install metadata (' + (legacyVersion || 'unknown') + ' -> ' + currentVersion + ')');
} catch (e) {
...
# ── Summary ─────────────────────────────────────────────────────
echo ""
echo "✓ ALOE updated successfully!"
echo ""
...
if systemctl --user is-active openclaw-gateway.service >/dev/null 2>&1 || \
  ...
  if systemctl --user restart openclaw-gateway.service 2>/dev/null; then
    ...
    if $RESTART_OK; then
      echo "  ✓ Gateway restarted — ALOE active on port 8402"
    else
...
echo ""
echo "  CLI commands:"
echo "    npx aloe report            # daily usage report"
echo "    npx aloe report weekly      # weekly report"
echo "    npx aloe report monthly     # monthly report"
echo "    npx aloe doctor             # AI diagnostics"
echo ""
