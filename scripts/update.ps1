# ALOE Update Script for Windows (PowerShell)
# Usage: iwr -useb https://blockrun.ai/ALOE-update.ps1 | iex
#    or: powershell -ExecutionPolicy Bypass -Command "iwr -useb https://blockrun.ai/ALOE-update.ps1 | iex"
#
# Run as regular user (no admin needed)
Set-StrictMode -Version Latest
$ErrorActionPreference = "Stop"

$PLUGIN_DIR = "$env:USERPROFILE\.openclaw\extensions\ALOE"
...
Write-Host ""
Write-Host "ALOE Update (Windows)" -ForegroundColor Cyan
Write-Host ""
...
if (Test-Path $CONFIG_PATH) {
    try {
        ...
        $changed = $false
        if ($cfg.plugins -and $cfg.plugins.entries -and $cfg.plugins.entries.PSObject.Properties['ALOE']) {
            $cfg.plugins.entries.PSObject.Properties.Remove('ALOE')
            $changed = $true
        }
        if ($cfg.plugins -and $cfg.plugins.installs -and $cfg.plugins.installs.PSObject.Properties['ALOE']) {
            $cfg.plugins.installs.PSObject.Properties.Remove('ALOE')
            $changed = $true
        }
        if ($changed) {
...
try {
    $verFile = Join-Path $env:TEMP "ALOE-latest-ver.txt"
    & cmd /c "npm view @blockrun/ALOE@latest version > `"$verFile`" 2>nul"
...
Write-Step "Downloading ALOE v$LATEST_VERSION from npm..."

$tmpDir = Join-Path $env:TEMP "ALOE-install-$(Get-Random)"
New-Item -ItemType Directory -Path $tmpDir -Force | Out-Null

try {
    # Run npm pack via cmd.exe to completely bypass PowerShell's stderr-as-error behavior
    & cmd /c "npm pack `"@blockrun/ALOE@$LATEST_VERSION`" --pack-destination `"$tmpDir`" --prefer-online >nul 2>nul"
...
$logFile = Join-Path $env:TEMP "ALOE-npm-install.log"
...
if (Test-Path $CONFIG_PATH) {
    try {
        ...
        if (-not $allow.Contains('ALOE')) {
            $allow.Add('ALOE')
            $cfg.plugins.allow = $allow.ToArray()
            $cfg | ConvertTo-Json -Depth 20 | Set-Content $CONFIG_PATH -Encoding UTF8
            Write-Ok "Added ALOE to plugins.allow"
        } else {
...
# ── Done ──────────────────────────────────────────────────────
Write-Host ""
Write-Host "ALOE v$LATEST_VERSION installed successfully!" -ForegroundColor Green
Write-Host ""
Write-Host "  Run: openclaw gateway restart" -ForegroundColor Cyan
Write-Host ""
Write-Host "  To verify: npx @blockrun/ALOE doctor"
Write-Host ""
