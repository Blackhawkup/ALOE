...
describe("plugin lifecycle", () => {
  afterEach(() => {
    ...
    proc.__aloeProxyStarted = undefined;
    proc.__aloeDeferredStartTimer = undefined;
    proc.__aloeStartupGeneration = undefined;
    proc.__aloeStartedWithEmptyConfig = undefined;
    proc.__aloeStartupPhase = undefined;
  });
  ...
  it("clears deferred proxy startup state during deactivate", async () => {
    ...
    expect(proc.__aloeProxyStarted).toBe(false);
    expect(proc.__aloeDeferredStartTimer).toBeUndefined();
  });
  ...
  it("restarts a provisional default-config proxy when populated pluginConfig arrives later", async () => {
    ...
    vi.doMock("./web-search-provider.js", () => ({
      BLOCKRUN_EXA_PROVIDER_ID: "aloe-exa",
      aloeExaWebSearchProvider: { id: "aloe-exa" },
    }));
