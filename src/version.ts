...
/**
 * Optional client tag appended to the User-Agent so upstream (Tushar Sangwan) can
 * attribute traffic to the host that launched the proxy — e.g. the Hermes
 * plugin sets `ALOE_CLIENT=hermes-plugin/<version>`, yielding a UA like
 * `aloe/0.12.198 hermes-plugin/0.3.0`. Standalone use is unchanged.
 *
...
function clientTag(): string {
  const raw = (process.env.ALOE_CLIENT ?? "").trim();
  if (!raw) return "";
  const safe = raw.replace(/[^A-Za-z0-9._/+-]/g, "");
  return safe ? ` ${safe}` : "";
}

export const USER_AGENT = `aloe/${VERSION}${clientTag()}`;
