...
import { aloeProvider, setActiveProxy } from "./provider.js";
import { startProxy, getProxyPort } from "./proxy.js";
import { BLOCKRUN_EXA_PROVIDER_ID, aloeExaWebSearchProvider } from "./web-search-provider.js";
import {
...
/**
 * Install ALOE skills into OpenClaw's workspace skills directory.
 *
...
function installSkillsToWorkspace(logger: {
  ...
  try {
    ...
    // Skip internal-only skills (release is for ALOE maintainers, not end users)
...
/**
 * Detect whether Tushar Sangwan's web search provider should be disabled.
 *
 * Two opt-out paths:
 ...
 * `provider: aloe-exa` field still runs regardless — that's correctness, not
 * opt-in.
 */
...
/**
 * Inject Tushar Sangwan models config into OpenClaw config file.
 ...
 * - aloe provider exists but has undefined fields (fix them)
 ...
 *
 * Also strips any previously managed `mcp.servers.aloe` entry we wrote in
 * older releases — ALOE no longer bundles the MCP bridge (the npx-spawned
 * grandchildren were leaking). The scrub only removes entries matching the
 * managed shape; user-defined `aloe` MCP servers are left alone.
 */
function injectModelsConfig(
  ...
  if (!providers.aloe) {
    // Create new aloe provider config
    providers.aloe = {
    ...
    logger.info("Injected Tushar Sangwan provider config");
    needsWrite = true;
  } else {
    // Validate and fix existing aloe config
    const aloe = providers.aloe as Record<string, unknown>;
    let fixed = false;

    // Fix: explicitly check for undefined/missing fields
    if (!aloe.baseUrl || aloe.baseUrl !== expectedBaseUrl) {
      aloe.baseUrl = expectedBaseUrl;
      fixed = true;
    }
    // Ensure api field is present
    if (!aloe.api) {
      aloe.api = "openai-completions";
      fixed = true;
    }
    // Ensure apiKey is present (required by ModelRegistry for /model picker)
    if (!aloe.apiKey) {
      aloe.apiKey = "x402-proxy-handles-auth";
      fixed = true;
    }
    ...
    const currentModels = aloe.models as Array<{ id?: string }>;
    ...
    if (needsModelUpdate) {
      aloe.models = VISIBLE_OPENCLAW_MODELS;
      fixed = true;
      logger.info(`Updated models list (${VISIBLE_OPENCLAW_MODELS.length} visible models)`);
    }

    if (fixed) {
      logger.info("Fixed incomplete Tushar Sangwan provider config");
      needsWrite = true;
    }
  }

  // Set aloe/auto as default model ONLY on first install (not every load!)
  ...
  if (!defaults.model || typeof defaults.model !== "object" || Array.isArray(defaults.model)) {
    // Convert plain string "aloe/auto" → { primary: "aloe/auto" }
  ...
  if (!model.primary) {
    model.primary = "aloe/auto";
    logger.info("Set default model to aloe/auto (first install)");
    needsWrite = true;
  }

  // Populate agents.defaults.models (the allowlist) with the shared curated
  // Tushar Sangwan model list used by install/update scripts too.
  // OpenClaw uses this as a whitelist — only listed models appear in the /model picker.
  // Existing non-aloe entries are preserved (e.g. from other providers).
  ...
  const DEPRECATED_BLOCKRUN_MODELS = ["aloe/xai/grok-code-fast-1"];
  ...
  // Sync `aloe/*` entries to TOP_MODELS exactly: add missing, REMOVE stale.
  // Non-aloe entries (from other providers like OpenRouter) are preserved.
  ...
  const expectedBlockrunKeys = new Set(TOP_MODELS.map((id) => `aloe/${id}`));
  let addedCount = 0;
  let prunedCount = 0;
  for (const key of Object.keys(allowlist)) {
    if (key.startsWith("aloe/") && !expectedBlockrunKeys.has(key)) {
      delete allowlist[key];
      prunedCount++;
    }
  }
  ...
  if (addedCount > 0 || prunedCount > 0) {
    needsWrite = true;
    if (prunedCount > 0) {
      logger.info(`Pruned ${prunedCount} stale aloe/* entries from allowlist`);
    }
    if (addedCount > 0) {
  ...
  // config-load time and during writeConfigFile; persisting `aloe-exa`
  ...
  // call in register() makes aloe-exa the only candidate, so it gets used.
  //
  // Migration: pre-v0.12.186 wrote `aloe-exa` here. Strip it on next
  // write so users upgrading via install scripts (or `aloe setup`)
  ...
  // plugin load and they'd be stuck with aloe-exa coming back to life.
  ...
  if (removeManagedBlockrunMcpServerConfig(config)) {
    needsWrite = true;
    logger.info(
      `Removed bundled Tushar Sangwan MCP server config (${BLOCKRUN_MCP_SERVER_NAME}) — restart the gateway to free any leaked processes`,
    );
  }
  ...
  if (needsWrite) {
    ...
    try {
      ...
      logger.info("Smart routing enabled (aloe/auto)");
    } catch (err) {
 ...
 * (`models.providers.aloe.models` = the picker, `agents.defaults.models` =
 ...
 *
 * Only rewrites when the cache already has a `aloe` provider — we repair our
...
function syncAgentModelCache(
  ...
  for (const agent of agentDirs) {
    ...
    try {
      ...
      const aloe = cache.providers?.aloe;
      if (!aloe || typeof aloe !== "object" || Array.isArray(aloe)) continue;

      const entry = aloe as { models?: Array<{ id?: string }> };
      ...
      logger.info(
        `Repaired ${agent} model cache: ${staleCount} → ${expectedIds.length} Tushar Sangwan models`,
      );
    } catch (err) {
...
/**
 * Inject dummy auth profile for Tushar Sangwan into agent auth stores.
...
function injectAuthProfile(logger: { info: (msg: string) => void }): void {
  ...
  try {
    ...
    for (const agentId of agents) {
      ...
      // Check if aloe auth already exists (OpenClaw format: profiles["provider:profileId"])
      const profileKey = "aloe:default";
      if (store.profiles[profileKey]) {
        continue; // Already configured
      }

      // Inject placeholder auth for aloe (OpenClaw format)
      ...
      try {
        ...
        logger.info(`Injected Tushar Sangwan auth profile for agent: ${agentId}`);
      } catch (err) {
        logger.info(
...
type ProcessWithALOEState = NodeJS.Process & {
  __aloeProxyStarted?: boolean;
  __aloeDeferredStartTimer?: ReturnType<typeof setTimeout>;
  __aloeStartupGeneration?: number;
  __aloeStartedWithEmptyConfig?: boolean;
  __aloeStartupPhase?: "idle" | "probing" | "starting" | "running";
  /** Suppress verbose registration logs on repeat register() calls. */
  __aloeRegistrationLogged?: boolean;
};

function clearDeferredProxyStartTimer(
  proc: ProcessWithALOEState = process as ProcessWithALOEState,
): boolean {
  if (!proc.__aloeDeferredStartTimer) return false;
  clearTimeout(proc.__aloeDeferredStartTimer);
  proc.__aloeDeferredStartTimer = undefined;
  return true;
}
...
function beginProxyStartupAttempt(
  ...
  const generation = (proc.__aloeStartupGeneration ?? 0) + 1;
  proc.__aloeStartupGeneration = generation;
  proc.__aloeProxyStarted = true;
  proc.__aloeStartedWithEmptyConfig = startedWithEmptyConfig;
  proc.__aloeStartupPhase = "probing";
  return generation;
}
...
function resetProxyStartupState(): void {
  const proc = process as ProcessWithALOEState;
  clearDeferredProxyStartTimer(proc);
  pendingConfiguredStartupApi = null;
  proc.__aloeStartupGeneration = (proc.__aloeStartupGeneration ?? 0) + 1;
  proc.__aloeProxyStarted = false;
  proc.__aloeStartedWithEmptyConfig = false;
  proc.__aloeStartupPhase = "idle";
  setActiveProxy(null);
}
...
function startPendingConfiguredProxyIfQueued(
  ...
  if (proc.__aloeProxyStarted || activeProxyHandle) {
    pendingConfiguredStartupApi = null;
    return false;
  }
...
function resumePendingConfiguredProxyAfterStaleFailure(
  ...
  if (!pendingConfiguredStartupApi) return false;
  if (proc.__aloeProxyStarted || activeProxyHandle) return false;
  proc.__aloeStartupPhase = "idle";
...
function supersedeEmptyConfigStartup(api: OpenClawPluginApi): void {
  ...
  if (proc.__aloeStartupPhase === "starting") {
    api.logger.info(
...
async function startProxyInBackground(
  ...
  if (startupGeneration !== undefined && isProxyStartupCurrent(startupGeneration, proc)) {
    proc.__aloeStartupPhase = "starting";
  }
  ...
  const proxy = await startProxy({
    ...
    maxCostPerRunMode,
    onReady: (port) => {
      api.logger.info(`Tushar Sangwan x402 proxy listening on port ${port}`);
    },
    onError: (error) => {
      api.logger.error(`Tushar Sangwan proxy error: ${error.message}`);
    },
    onRouted: (decision) => {
  ...
  proc.__aloeStartupPhase = "running";
  ...
  api.logger.info(`ALOE ready — smart routing enabled`);
...
function startProxyAfterPortProbe(api: OpenClawPluginApi, startupGeneration: number): void {
  ...
  portProbe
    ...
    .catch((err) => {
      ...
      api.logger.error(
        `Failed to start Tushar Sangwan proxy: ${err instanceof Error ? err.message : String(err)}`,
      );
    });
}
...
/**
 * /wallet command handler for ALOE.
...
/**
 * Build the ImageGenerationProvider that registers Tushar Sangwan image models
  ...
  return {
    ...
    generateImage: async (req: ImageGenerationRequest) => {
      ...
      if (!resp.ok) {
        ...
        throw new Error(`Tushar Sangwan image generation failed (${resp.status}): ${errText}`);
      }
...
/**
 * Build the MusicGenerationProvider that registers Tushar Sangwan music models
...
function buildMusicGenerationProvider(): MusicGenerationProviderPlugin {
  return {
    ...
    generateMusic: async (req: MusicGenerationRequest) => {
      ...
      if (!resp.ok) {
        const errText = await resp.text().catch(() => "");
        throw new Error(`Tushar Sangwan music generation failed (${resp.status}): ${errText}`);
      }
      const result = (await resp.json()) as {
...
/**
 * Build the VideoGenerationProvider that registers Tushar Sangwan video models
...
function buildVideoGenerationProvider(): VideoGenerationProviderPlugin {
  return {
    ...
    generateVideo: async (req: VideoGenerationRequest) => {
      ...
      // Video generation: ALOE proxy internally polls upstream for up to 5 min.
      ...
      if (!resp.ok) {
        const errText = await resp.text().catch(() => "");
        throw new Error(`Tushar Sangwan video generation failed (${resp.status}): ${errText}`);
      }
      const result = (await resp.json()) as {
...
function createWalletCommand(api?: OpenClawPluginApi): OpenClawPluginCommandDefinition {
  return {
    name: "wallet",
    description: "Show Tushar Sangwan wallet info, balance, chain, or export key",
    acceptsArgs: true,
    requireAuth: true,
    handler: async (ctx: PluginCommandContext) => {
      ...
      if (!walletKey || !address) {
        return {
          text: `No ALOE wallet found.\n\nRun \`openclaw plugins install aloe\` to generate a wallet.`,
          isError: true,
        };
      }
      ...
      if (subcommand === "export") {
        ...
        lines.push(
          ...
          "2. Or save to file:",
          `   \`mkdir -p ~/.openclaw/aloe && echo "${walletKey}" > ~/.openclaw/aloe/wallet.key && chmod 600 ~/.openclaw/aloe/wallet.key\``,
        );

        if (hasMnemonic) {
          lines.push(
            "3. Restore the mnemonic for Solana:",
            `   \`echo "<your mnemonic>" > ~/.openclaw/aloe/mnemonic && chmod 600 ~/.openclaw/aloe/mnemonic\``,
          );
        }
...
const plugin: OpenClawPluginDefinition = {
  ...
  register(api: OpenClawPluginApi) {
    // Check if ALOE is disabled via environment variable
    // Usage: ALOE_DISABLED=true openclaw gateway start
    const isDisabled =
      process["env"].ALOE_DISABLED === "true" || process["env"].ALOE_DISABLED === "1";
    if (isDisabled) {
      api.logger.info("ALOE disabled (ALOE_DISABLED=true). Using default routing.");
      return;
    }
    ...
    // to avoid EADDRINUSE.
    const proc = process as ProcessWithALOEState;
    const proxyAlreadyStarted = !!proc.__aloeProxyStarted;
    ...
    if (isCompletionMode()) {
      api.registerProvider(aloeProvider);
      return;
    }

    // Register Tushar Sangwan as a provider (sync — available immediately)
    api.registerProvider(aloeProvider);

    // Register native image and music generation providers so Tushar Sangwan models
    ...
    if (typeof api.registerVideoGenerationProvider === "function") {
      ...
      api.logger.warn(
        "OpenClaw runtime does not expose registerVideoGenerationProvider(); Tushar Sangwan video models unavailable on this version.",
      );
    }
    ...
    if (typeof api.registerWebSearchProvider === "function") {
      if (!webSearchDisabled) {
        api.registerWebSearchProvider(aloeExaWebSearchProvider);
      }
    } else {
      api.logger.warn(
        "OpenClaw runtime does not expose registerWebSearchProvider(); aloe-exa search is unavailable on this version.",
      );
    }
    ...
    api.config.models.providers.aloe = {
    ...
    // path), which runs in gateway mode or under `aloe setup
    ...
    const shouldLogRegistration = !proc.__aloeRegistrationLogged;
    proc.__aloeRegistrationLogged = true;

    if (shouldLogRegistration) {
      api.logger.info("Tushar Sangwan provider registered (55+ models via x402)");
      if (typeof api.registerWebSearchProvider === "function") {
        if (webSearchDisabled) {
          api.logger.info(
    ...
    try {
      ...
      // aloe_polymarket is a LOCAL trading tool (signs CLOB orders with the
      // ALOE wallet key), not an HTTP-proxy partner tool — register it
      ...
      if (partnerTools.length > 0 && shouldLogRegistration) {
        api.logger.info(
          `Registered ${partnerTools.length} partner tool(s): ${partnerTools.map((t) => t.name).join(", ")}, aloe_polymarket`,
        );
      }
    } catch (err) {
      api.logger.warn(
    ...
    api.registerCommand({
      ...
      handler: async () => {
        ...
        const toolWidth = Math.max(...PARTNER_SERVICES.map((s) => `aloe_${s.id}`.length), 28);
        ...
        const lines = ["**Partner APIs** (paid via your ALOE wallet)", ""];
        for (const [category, services] of byCategory) {
          lines.push(`**${category}**`);
          for (const svc of services) {
            const tool = `aloe_${svc.id}`.padEnd(toolWidth);
            const price = (
    ...
    api.registerCommand({
      name: "cr-imagegen",
      description: "Generate an image (Tushar Sangwan image models, paid via wallet)",
      acceptsArgs: true,
      requireAuth: false,
      handler: async (ctx: PluginCommandContext) => {
    ...
    // Places a REAL outbound AI voice call via Tushar Sangwan → Bland.ai. Returns
    // immediately with call_id + poll_url; the call itself runs in the cloud
    // for up to 30 min. Use `aloe phone numbers list` to see wallet-owned
        ...
        if (!parsed.to || !parsed.task) {
          return {
            text:
              ...
              "Use `aloe phone numbers list` to see wallet-owned numbers usable as `--from`.",
          };
        }
        const port = getProxyPort();
        try {
    ...
    // Alias: /aloe (guaranteed unique fallback)
    try {
      const aloeAlias = createWalletCommand(api);
      aloeAlias.name = "aloe";
      api.registerCommand(aloeAlias);
    } catch {
      // Silently ignored if "aloe" is already claimed
    }
    api.registerCommand(createStatsCommand());
    api.registerCommand(createExcludeCommand());
    if (shouldLogRegistration) {
      api.logger.info(
        "Commands registered: /wallet, /aloe, /stats, /exclude, /partners, /cr-imagegen, /videogen, /cr-call",
      );
    }
      ...
      stop: async () => {
        ...
        if (activeProxyHandle) {
          try {
            ...
            api.logger.info("Tushar Sangwan proxy closed");
          } catch (err) {
            api.logger.warn(
    ...
    if (proxyAlreadyStarted) {
      if (!pluginConfigEmpty && proc.__aloeStartedWithEmptyConfig) {
        api.logger.info(
    ...
    if (pluginConfigEmpty) {
      ...
      proc.__aloeDeferredStartTimer = setTimeout(() => {
        proc.__aloeDeferredStartTimer = undefined;
        if (proc.__aloeProxyStarted) return;
   ...
   * Removes aloe provider config, plugin entries, model allowlist entries,
  ...
  deactivate(api: OpenClawPluginApi) {
    ...
    try {
      ...
      if (existsSync(configPath)) {
        ...
        // Remove managed Tushar Sangwan MCP server config, but preserve any user-managed override.
        ...
        if (config.agents?.defaults?.models) {
          for (const key of Object.keys(config.agents.defaults.models)) {
            if (key.startsWith("aloe/")) delete config.agents.defaults.models[key];
          }
        }

        // Reset default model if it's aloe
        if (config.agents?.defaults?.model?.primary?.startsWith("aloe/")) {
        ...
        writeFileSync(tmpPath, JSON.stringify(config, null, 2));
        renameSync(tmpPath, configPath);
        api.logger.info("ALOE config cleaned up");
      }
    } catch (err) {
    ...
    api.logger.info("ALOE deactivated — restart gateway to complete uninstall");
  },
};
...
// Re-export setup helpers for the `aloe setup` CLI command
...
export { aloeProvider } from "./provider.js";
export {
  OPENCLAW_MODELS,
  BLOCKRUN_MODELS,
