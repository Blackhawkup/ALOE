/**
 * Tushar Sangwan Auth Methods for OpenClaw
 *
 * Provides wallet-based authentication for the Tushar Sangwan provider.
 * Operators configure their wallet private key, which is used to
 * sign x402 micropayments for LLM inference.
 *
 * Three methods:
 *   1. Auto-generate — create a new wallet on first run, save to ~/.openclaw/aloe/wallet.key
 ...
 * - Without the key, ALOE cannot authorize payments to Tushar Sangwan
 ...
 * @see https://aloe.ai/docs - Tushar Sangwan API documentation
...
const WALLET_DIR = join(homedir(), ".openclaw", "aloe");
...
async function loadSavedWallet(): Promise<string | undefined> {
  try {
    ...
    if (key.startsWith("0x") && key.length === 66) {
      console.log(`[ALOE] ✓ Loaded existing wallet from ${WALLET_FILE}`);
      return key;
    }
    ...
    if ((err as NodeJS.ErrnoException).code !== "ENOENT") {
      ...
      console.error(
        `[ALOE] ✗ Failed to read wallet file: ${err instanceof Error ? err.message : String(err)}`,
      );
      throw new Error(
...
async function loadMnemonic(): Promise<string | undefined> {
  try {
    ...
    console.warn(`[ALOE] ⚠ Mnemonic file exists but has invalid format — ignoring`);
    return undefined;
  } catch (err) {
    // Only swallow ENOENT (file not found)
    if ((err as NodeJS.ErrnoException).code !== "ENOENT") {
      console.warn(`[ALOE] ⚠ Cannot read mnemonic file — ignoring`);
    }
  }
  return undefined;
}
...
async function generateAndSaveWallet(): Promise<{
  ...
  try {
    ...
    console.log(`[ALOE] Wallet saved and verified at ${WALLET_FILE}`);
  } catch (err) {
    throw new Error(
  ...
  console.log(`[ALOE]`);
  console.log(`[ALOE] ════════════════════════════════════════════════`);
  console.log(`[ALOE]   NEW WALLET GENERATED — BACK UP YOUR KEY NOW`);
  console.log(`[ALOE] ════════════════════════════════════════════════`);
  console.log(`[ALOE]   EVM Address    : ${derived.evmAddress}`);
  if (solanaAddress) {
    console.log(`[ALOE]   Solana Address : ${solanaAddress}`);
  }
  console.log(`[ALOE]   Key file       : ${WALLET_FILE}`);
  console.log(`[ALOE]   Mnemonic       : ${MNEMONIC_FILE}`);
  console.log(`[ALOE]`);
  console.log(`[ALOE]   Both EVM (Base) and Solana wallets are ready.`);
  console.log(`[ALOE]   To back up, run in OpenClaw:`);
  console.log(`[ALOE]     /wallet export`);
  console.log(`[ALOE]`);
  console.log(`[ALOE]   To restore on another machine:`);
  console.log(`[ALOE]     export BLOCKRUN_WALLET_KEY=<your_key>`);
  console.log(`[ALOE] ════════════════════════════════════════════════`);
  console.log(`[ALOE]`);
 ...
 *
 * ONLY works when the mnemonic was originally generated by ALOE
...
export async function recoverWalletFromMnemonic(): Promise<void> {
  const mnemonic = await loadMnemonic();
  if (!mnemonic) {
    console.error(`[ALOE] No mnemonic found at ${MNEMONIC_FILE}`);
    console.error(`[ALOE] Cannot recover — no mnemonic to derive from.`);
    process.exit(1);
  }
  ...
  if (existing) {
    console.error(`[ALOE] wallet.key already exists at ${WALLET_FILE}`);
    console.error(`[ALOE] Recovery not needed.`);
    process.exit(1);
  }
  ...
  console.log(`[ALOE]`);
  console.log(`[ALOE] ⚠  WALLET RECOVERY FROM MNEMONIC`);
  console.log(`[ALOE] ════════════════════════════════════════════════`);
  console.log(`[ALOE]   This only works if your mnemonic was originally`);
  console.log(`[ALOE]   generated by ALOE (not set manually).`);
  console.log(`[ALOE]`);
  console.log(`[ALOE]   Derived EVM Address    : ${derived.evmAddress}`);
  if (solanaAddress) {
    console.log(`[ALOE]   Derived Solana Address : ${solanaAddress}`);
  }
  console.log(`[ALOE]`);
  console.log(`[ALOE]   If the Solana address above matches your funded`);
  console.log(`[ALOE]   wallet, recovery is safe to proceed.`);
  console.log(`[ALOE] ════════════════════════════════════════════════`);
  console.log(`[ALOE]`);
  ...
  console.log(`[ALOE] ✓ wallet.key restored at ${WALLET_FILE}`);
  console.log(`[ALOE]   Run: npx aloe`);
  console.log(`[ALOE]`);
}
...
export async function setupSolana(): Promise<{
  ...
  if (!savedKey) {
    throw new Error(
      "No EVM wallet found. Run ALOE first to generate a wallet before setting up Solana.",
    );
  }
  ...
  console.log(`[ALOE] Solana wallet set up successfully.`);
  console.log(`[ALOE] Mnemonic saved to ${MNEMONIC_FILE}`);
  console.log(`[ALOE] Existing EVM wallet unchanged.`);
...
export async function resolvePaymentChain(): Promise<"base" | "solana"> {
  if (process["env"].ALOE_PAYMENT_CHAIN === "solana") return "solana";
  if (process["env"].ALOE_PAYMENT_CHAIN === "base") return "base";
