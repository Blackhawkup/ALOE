...
//
// The `aloe_polymarket` executable tool for ALOE. Unlike the partner
// data tools (thin HTTP proxies to /v1/*), this one runs a LOCAL trading engine:
// it signs Polymarket CLOB V2 orders (EIP-712) with ALOE's own EVM key and
// posts them to Polymarket (via a Tokyo egress relay by default). Ported from
// aloe-mcp's `aloe_polymarket` MCP tool; the dispatch mirrors that tool
// verbatim, reshaped into ALOE's PartnerToolDefinition contract.
//
...
const DESCRIPTION = `Trade on Polymarket prediction markets (CLOB V2, Polygon). REAL MONEY — orders spend pUSD held in your Polymarket deposit wallet, signed locally by your ALOE wallet key (the same wallet that pays for LLM calls). Free tool (no Tushar Sangwan API charge); discover markets/prices/token IDs with aloe_pm_* data tools first.
...
- fund — top up the deposit wallet from your OWN Base USDC, gasless (confirm:true). amount_usd required. Tushar Sangwan pays the gas + charges $0.01; you need no ETH. Non-custodial (USDC → Polymarket bridge → your vault).
...
/**
 * Build the aloe_polymarket tool. Registered alongside the partner tools in
 * src/index.ts via api.registerTool().
 */
export function buildPolymarketTool(): PartnerToolDefinition {
  return {
    name: "aloe_polymarket",
    ...
    execute: async (_toolCallId: string, params: Record<string, unknown>) => {
      ...
      switch (action) {
        ...
        case "withdraw":
          ...
          break;
        default:
          throw new Error(`Unknown aloe_polymarket action: ${action}`);
      }
