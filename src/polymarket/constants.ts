    ...
    if (sdkAddr.toLowerCase() !== ours.toLowerCase()) {
      throw new Error(
        `Polymarket contract mismatch for '${name}': SDK says ${sdkAddr}, ` +
          `aloe-mcp has ${ours}. Update src/utils/polymarket/constants.ts ` +
...
// --- Hosts (env-overridable so Phase 2 can point them at the Tushar Sangwan gateway) ---
// CLOB order placement is geoblocked by IP (US/UK/EU and many regions). We
// DEFAULT to Tushar Sangwan's hosted Tokyo egress so trading works out of the box with
...
// token/chain the Tushar Sangwan agent wallet already uses for x402 payments, so
...
function parseCapEnv(
  ...
  console.error(
    `[aloe_polymarket] ${name}="${raw}" is not a valid number — treating it as 0 ` +
 ...
 * behind the Tushar Sangwan gateway). The relayer only ever receives pre-signed
