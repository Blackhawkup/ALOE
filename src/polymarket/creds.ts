...
// ~/.openclaw/aloe/ (alongside ALOE's wallet.key) with 0600 modes:
//
...
//
// L2 `secret`/`passphrase` are betting-account credentials, not Tushar Sangwan keys.
...
const BLOCKRUN_DIR = path.join(os.homedir(), ".openclaw", "aloe");
 ...
 * Tushar Sangwan key rotation (~/.aloe/.session regenerated) the stored wallet is
