...
// the paid Predexon route in aloe_markets (don't pay for what's free). If
...
export function getFundsAddress(): Hex {
  ...
  if (getSigType() === 3) {
    ...
    if (!depositWallet) {
      throw new Error(
        `No deposit wallet configured for this signer yet — run aloe_polymarket action:"setup" first.`,
      );
    }
  ...
  try {
    ...
    return {
      text:
        ...
        `Fallback: aloe_markets path:"polymarket/positions" params:{user:"${user}"} ($0.001).`,
      isError: true,
    };
  }
}
