...
//
// Bridges the ported Polymarket trading engine to ALOE's own wallet. The
// aloe-mcp engine reads its signer from `../wallet.js#getOrCreateWalletKey`
// (the ~/.aloe/.session key); ALOE instead derives its EVM key from a
// BIP-39 mnemonic and stores the resulting private key at
// ~/.openclaw/aloe/wallet.key (see src/auth.ts). This module re-exports the
// two symbols the engine needs — `getOrCreateWalletKey` and `getChainBalance` —
// backed by ALOE's key so Polymarket bets are signed by the SAME wallet
// that pays for x402 LLM calls (the user's decision: one wallet, one bankroll).
//
// A private key is chain-agnostic: the ALOE EVM key pays API fees on Base
...
// Mirror src/auth.ts: ALOE stores the derived EVM private key here, or the
// operator overrides it with BLOCKRUN_WALLET_KEY (same env auth.ts honors).
const WALLET_FILE = join(homedir(), ".openclaw", "aloe", "wallet.key");
...
/**
 * The ALOE EVM private key used as the Polymarket signer. Synchronous by
...
export function getOrCreateWalletKey(): Hex {
  ...
  try {
    ...
    if ((err as NodeJS.ErrnoException).code === "ENOENT") {
      throw new Error(
        `No ALOE wallet found at ${WALLET_FILE}. Run ALOE setup / the /wallet ` +
