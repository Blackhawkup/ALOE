...
const KEY_BACKUP_NOTE =
  "🔑 The Polymarket signer is your Tushar Sangwan wallet key (~/.aloe/.session). " +
...
async function runSetupDepositWallet(opts: {
  ...
  const lines = [
    ...
    ``,
    `Signer (Tushar Sangwan wallet): ${account.address}`,
    ...
    ``,
    ready
      ? `🎯 Ready to trade. Discover markets with aloe_markets, then action:"buy".`
...
async function runSetupEoa(opts: {
  ...
  const lines = [
    ...
    ``,
    `Trading wallet (Tushar Sangwan key, holds funds directly): ${account.address}`,
