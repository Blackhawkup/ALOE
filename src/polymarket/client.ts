...
// local EVM key that pays Tushar Sangwan x402 fees on Base (~/.aloe/.session) —
...
/** The local EOA account (Tushar Sangwan session key) used as the Polymarket signer. */
 ...
 * calls) share via the hoisted axios install; @aloe/llm and the other tools
...
export async function getClobClient(): Promise<ClobClient> {
  ...
  if (sigType === 3 && !depositWallet) {
    throw new Error(
      `No Polymarket deposit wallet configured for this signer yet. Run aloe_polymarket ` +
