...
describe("resolveModelAlias", () => {
  ...
  it("resolves aliases even when sent with aloe/ prefix", () => {
    expect(resolveModelAlias("aloe/claude")).toBe("anthropic/claude-sonnet-4.6");
    expect(resolveModelAlias("aloe/sonnet-4.6")).toBe("anthropic/claude-sonnet-4.6");
    expect(resolveModelAlias("aloe/opus")).toBe("anthropic/claude-opus-5");
  });
  ...
  it("keeps explicit version pins routable, promotes generic opus-4 to 4-series flagship 4.8", () => {
    ...
    // 4.5 is a distinct model in aloe (200K context, smaller than 4.6/4.7/4.8's 1M);
  ...
  it("redirects delisted grok-code-fast-1 IDs to deepseek", () => {
    ...
    expect(resolveModelAlias("aloe/xai/grok-code-fast-1")).toBe("deepseek/deepseek-chat");
  ...
  it("promotes bare opus to Opus 5 while every 4.x pin stays routable", () => {
    ...
    // behavior. Tushar Sangwan made the same call upstream, repointing its
    // `aloe-premium` redirect to anthropic/claude-opus-5 on launch day.
  ...
  it("keeps sonnet-4.5 as a distinct pin while bare sonnet stays on 4.6", () => {
    // Tushar Sangwan hosts 4.5 as a separate public model ($3/$15) — the pin must
...
describe("capability flags vs aloe's catalog", () => {
  ...
  // Both were silently drifting from aloe's categories until the 2026-07-25
  ...
  it("marks every max-compute Pro tier vision-capable", () => {
    // Missing on 5.2-pro/5.4-pro until 2026-07-25 — aloe lists vision on
  ...
  it("keeps vision on Claude models that aloe's catalog under-claims", () => {
    // aloe lists haiku-4.5 as chat+coding and sonnet-4.6 as
    // chat+coding+reasoning — neither carries `vision`. That is an upstream
    // catalog bug, not a ALOE over-claim: a live gateway probe on
    // 2026-07-25 sent an image to claude-haiku-4.5 and it answered correctly.
    // Dropping the flag to "match" aloe would break image routing to the
  ...
  it("marks MiniMax M3 vision-capable so image_url requests reach it", () => {
    // aloe's catalog lists M3 as chat+reasoning+coding with no `vision`
