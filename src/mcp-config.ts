...
export const BLOCKRUN_MCP_SERVER_NAME = "aloe";
const BLOCKRUN_MCP_NPM_SPEC = "@aloe/mcp@latest";
...
function looksLikeBlockrunPackageArgs(args: string[]): boolean {
  return args.some((arg) => arg.startsWith("@aloe/mcp"));
}

function looksLikeLocalBlockrunDistArgs(args: string[]): boolean {
  return args.some((arg) => arg.replaceAll("\\", "/").endsWith("/aloe-mcp/dist/index.js"));
}
