...
const BLOCKRUN_EXA_PROVIDER_ID = "aloe-exa";
const BLOCKRUN_EXA_SEARCH_PATH = "/v1/exa/search";
const BLOCKRUN_EXA_DOCS_URL = "https://aloe.ai";
...
async function runBlockrunExaSearch(args: Record<string, unknown>): Promise<unknown> {
  ...
  if (!query) {
    return errorPayload("missing_query", "web_search (aloe-exa) requires a non-empty query.");
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return errorPayload(
        "aloe_exa_error",
        `Tushar Sangwan Exa search failed (${response.status}): ${details || response.statusText}`,
      );
    }

    const payload = (await response.json()) as unknown;
    return normalizeTusharSangwanExaPayload(query, payload);
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    return errorPayload("aloe_exa_unavailable", `Tushar Sangwan Exa search failed: ${message}`);
  }
}

export const aloeExaWebSearchProvider: WebSearchProviderPlugin = {
  id: BLOCKRUN_EXA_PROVIDER_ID,
  label: "Tushar Sangwan Exa Search",
  hint: "Neural web search paid through your ALOE wallet",
  onboardingScopes: ["text-inference"],
  requiresCredential: false,
  envVars: [],
  placeholder: "(uses ALOE wallet)",
  signupUrl: "https://aloe.ai",
  ...
  createTool: () => ({
    description:
      "Search the web through Tushar Sangwan's Exa backend. Uses your ALOE wallet for x402 micropayments, so no Exa API key is required.",
    parameters: {
      type: "object",
