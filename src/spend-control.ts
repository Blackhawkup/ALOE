 ...
 *
 * Absorbed from @aloe/clawwallet. Chain-agnostic (works for both EVM and Solana).
 *
 * Features:
 ...
 * - Persistent storage (~/.openclaw/aloe/spending.json)
 */
...
const WALLET_DIR = path.join(homedir(), ".openclaw", "aloe");
...
export class FileSpendControlStorage implements SpendControlStorage {
  ...
  load(): { limits: SpendLimits; history: SpendRecord[] } | null {
    try {
      ...
      console.error(`[ALOE] Failed to load spending data, starting fresh: ${err}`);
    }
    return null;
  }
  ...
  save(data: { limits: SpendLimits; history: SpendRecord[] }): void {
    try {
      ...
      console.error(`[ALOE] Failed to save spending data: ${err}`);
    }
  }
}
