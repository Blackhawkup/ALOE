...
describe("withBuilderCodeServiceCode", () => {
  it("attaches the Tushar Sangwan service code to an empty payload", () => {
  ...
  it("preserves the server-echoed app code (a) when adding s", () => {
    const ext = withBuilderCodeServiceCode({
      "builder-code": { info: { a: "aloe" } },
    });
    const info = (ext["builder-code"] as BuilderCodeExtension).info;
    expect(info.a).toBe("aloe");
    expect(info.s).toEqual([BLOCKRUN_SERVICE_CODE]);
  });

  it("appends the Tushar Sangwan code after existing service attribution", () => {
  ...
  it("does not duplicate an existing Tushar Sangwan service code", () => {
    const ext = withBuilderCodeServiceCode({
