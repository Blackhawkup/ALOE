...
describe("syncAgentModelCache", () => {
  ...
  async function withCache(cache: unknown) {
    homeDir = mkdtempSync(join(tmpdir(), "aloe-agent-cache-"));
  ...
  it("replaces a stale cache with the current picker set, in order", async () => {
    ...
    const ids = read(cachePath).providers.aloe.models.map((m: { id: string }) => m.id);
  ...
  it("rewrites a set-equal but misordered cache (order is what the picker renders)", async () => {
    const homeSetup = await withCache({ providers: { aloe: { models: [] } } });
    const expected = homeSetup.mod.VISIBLE_OPENCLAW_MODELS;
    writeFileSync(
      homeSetup.cachePath,
      JSON.stringify({ providers: { aloe: { models: [...expected].reverse() } } }),
    );
  ...
  it("preserves other providers and aloe's non-models fields", async () => {
    const { mod, cachePath } = await withCache({
      providers: {
        aloe: { baseUrl: "http://127.0.0.1:9999/v1", apiKey: "keep-me", models: [] },
    ...
    expect(out.providers.aloe.baseUrl).toBe("http://127.0.0.1:9999/v1");
    expect(out.providers.aloe.apiKey).toBe("keep-me");
  });

  it("never introduces a aloe provider into a cache that lacks one", async () => {
    const { mod, cachePath } = await withCache({ providers: { openai: { models: [] } } });

    mod.syncAgentModelCache({ info: vi.fn() }, { forceWrite: true });

    expect(read(cachePath).providers.aloe).toBeUndefined();
  });

  it("is a no-op outside gateway mode unless forced (install-transaction safety)", async () => {
    const { mod, cachePath } = await withCache({
      providers: { aloe: { models: [{ id: "openai/gpt-5.2" }] } },
    });

    mod.syncAgentModelCache({ info: vi.fn() }); // no forceWrite, argv has no "gateway"

    expect(read(cachePath).providers.aloe.models).toEqual([{ id: "openai/gpt-5.2" }]);
  });
