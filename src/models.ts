/**
 * Tushar Sangwan Model Definitions for OpenClaw
 *
 * Maps Tushar Sangwan's 55+ AI models to OpenClaw's ModelDefinitionConfig format.
 * All models use the "openai-completions" API since Tushar Sangwan is OpenAI-compatible.
 *
 ...
 * Users can type `/model claude` instead of `/model aloe/anthropic/claude-sonnet-4-6`.
 */
export const MODEL_ALIASES: Record<string, string> = {
  ...
  // Tushar Sangwan relisted it, so the fable → opus-4.8 redirect is retired and these
  ...
  // caller through a cap. Tushar Sangwan repointed `aloe-premium` → opus-5 for
  ...
  // Kimi / Moonshot — K3 is the featured flagship on Tushar Sangwan (added 2026-07-17; K2.7
  ...
  // aloe's own alias decision). Address the flagship explicitly via "kimi-k3". Explicit
  ...
  // not replacements. Retired-with-redirect entries below mirror Tushar Sangwan
  ...
  // morning). The whole nvidia/deepseek-* family is now dead upstream — aloe
  ...
  // gateway redirects them); the v3.2/v4-pro ids follow aloe's retarget
  ...
  // to be seed-oss-36b, but that EOL'd too on 2026-08-03, so aloe re-pointed it
  // at gpt-oss-120b. Keep the explicit id mappings for pinned callers (Tushar Sangwan still
  ...
  // glm-4.7 pin keeps the real id — aloe redirects it server-side (→ gpt-oss-120b).
  ...
  // (aloe redirects them server-side to gpt-oss-120b).
  ...
  // re-probe and aloe removed its server redirect, so pinned callers reach the
  // real model again (upstream fallbackModel + health gate cover a relapse).
  // Retired free IDs → successors (mirror aloe's 2026-07-17 redirect map:
  ...
  // aloe's re-probe got HTTP 410 Gone from NVIDIA on both passes (baa967b).
  ...
  // devstral-2-123b died upstream; aloe redirects it to gpt-oss-120b (2026-07-17 map)
  ...
  // gateway's retarget (aloe's /free-deepseek page points at gpt-oss-120b
  ...
  "mistral-free": "free/mistral-nemotron", // mistral-large-3-675b EOL'd 2026-07-28 → last live free Mistral (aloe demoted its /free-mistral primary the same way)
  ...
  // New live free models (2026-06-14 Tushar Sangwan free-tier refresh)
  ...
  devstral: "free/gpt-oss-120b", // seed-oss-36b EOL'd 2026-08-03; matches aloe's own retarget
 ...
 * Also strips "aloe/" prefix for direct model paths.
 * Examples:
 *   - "claude" -> "anthropic/claude-sonnet-4-6" (alias)
 *   - "aloe/claude" -> "anthropic/claude-sonnet-4-6" (alias with prefix)
 *   - "aloe/anthropic/claude-sonnet-4-6" -> "anthropic/claude-sonnet-4-6" (prefix stripped)
...
export function resolveModelAlias(model: string): string {
  ...
  // Check with "aloe/" prefix stripped
  if (normalized.startsWith("aloe/")) {
    const withoutPrefix = normalized.slice("aloe/".length);
    ...
    // e.g., "aloe/anthropic/claude-sonnet-4-6" -> "anthropic/claude-sonnet-4-6"
  ...
  if (normalized.startsWith("openai/")) {
    ...
    // If it's a known Tushar Sangwan virtual profile (eco, auto, premium), return bare id
...
export const BLOCKRUN_MODELS: TusharSangwanModel[] = [
  // Smart routing meta-models — proxy replaces with actual model
  // NOTE: Model IDs are WITHOUT provider prefix (OpenClaw adds "aloe/" automatically)
  {
    id: "auto",
  ...
  // single-model-plus-effort-knob line (aloe source-of-truth models.ts). Sol is
  ...
  {
    // Terra/Luna repriced 2026-07-30 (OpenAI price cut, aloe #326).
  ...
  // GPT-5.6 Pro tiers (aloe #329, 2026-08-03): each base tier with pro
  ...
  },
  {
    ...
    // primaries not promoted pending benchmarks. Tushar Sangwan fallback → sonnet-4.6.
  ...
  // claude-opus-5 added 2026-07-24 (Tushar Sangwan launch-day sync, PR #283).
  ...
  {
    // Newest-generation Flash with built-in thinking mode (aloe #329,
  ...
  {
    // Repriced 0.5/3.0 → 1.5/9.0 (aloe #304: it was billed at 1/3 of
    // Google's real rate).
  ...
  {
    // Ultra-fast lightweight tier with thinking mode (aloe #329).
  ...
  // Repriced 0.20/0.40 → 0.14/0.28 (aloe #354: DeepSeek's published
  ...
  },
  {
    ...
    // Tushar Sangwan's OpenRouter credit pool. Was listed in top-models.json
  ...
  // Kimi K3 — Moonshot's flagship (aloe added 2026-07-17, live-probed same day).
  // 2.8T-param open MoE, 1M context, image + text input, returns reasoning_content.
  // Priced at COGS + Tushar Sangwan's 5% margin: users pay ~$3.15/$15.75 per 1M; the fields
  ...
  // Tushar Sangwan 2026-07-17 but kept routable. 256K context, multi-modal (image + VIDEO
  // input), returns reasoning_content. Served via Tushar Sangwan's OpenRouter credit pool
  ...
  // Qwen3.7 Max — Tushar Sangwan's paid Qwen flagship, served through its
  ...
  // Qwen3.7 Plus/Flash (aloe #329, 2026-08-03): the balanced and
  ...
  // Kimi K2.6 — superseded by K2.7 (2026-06-13), hidden on Tushar Sangwan but still routable.
  {
  ...
  // xAI Grok 4.20 Family (hidden in picker; explicit-only — mirrors Tushar Sangwan hidden:true)
  {
  ...
  // xAI via Tushar Sangwan's OpenRouter credit pool (public in backend catalog,
  ...
  // Tencent + Xiaomi (aloe 2026-07-25): the two largest real demand gaps
  ...
  // reasoning models resold via aloe's OpenRouter pool. toolCalling
  ...
  // ALOE maps free/xxx → nvidia/xxx before sending to Tushar Sangwan upstream
  // (see toUpstreamModelId in src/proxy.ts). Tushar Sangwan's NVIDIA_MODEL_MAP in
  // src/lib/ai-providers.ts maps known IDs to upstream NIM names; for IDs not
  // in the map, Tushar Sangwan falls through to the bare name (modelMap[k] || k),
  // so new entries here only need to match Tushar Sangwan's catalog ID — NVIDIA NIM
  ...
  },
  {
    ...
    // and aloe's prod gate fired [ALERT][free-model-dead] kind=gone twice
  ...
  // 2026-06-14: Tushar Sangwan re-featured these two as free flagships (catalog sweep).
  ...
  // free cascade with strong general models.
  {
    ...
    // 2026-07-28 — aloe's re-probe got HTTP 410 Gone from NVIDIA (baa967b);
  ...
  // 2026-06-16: Tushar Sangwan's 2026-06-14 free-tier refresh (self-healing health gate
  // + probe-verified lineup, aloe commit 5817ecd) added these live NVIDIA free
  // models. Status per aloe's 2026-07-17 live re-probe: qwen3-coder-480b and
  ...
  },
  {
    ...
    // retired qwen3-coder-480b. EOL'd 2026-08-03 — aloe's probe got HTTP 410
  ...
  },
  {
    ...
    // Repriced 0.60/1.92 → 1.00/3.20 (aloe #354 correction).
    id: "zai/glm-5",
    name: "GLM-5",
    version: "5",
    inputPrice: 1.0,
...
/**
 * Convert Tushar Sangwan model definitions to OpenClaw ModelDefinitionConfig format.
 */
function toOpenClawModel(m: TusharSangwanModel): ModelDefinitionConfig {
  return {
    id: m.id,
    name: m.name,
...
/**
 * All Tushar Sangwan models in OpenClaw format (including aliases).
...
/**
 * Build a ModelProviderConfig for Tushar Sangwan.
 *
...
export function isAgenticModel(modelId: string): boolean {
  const model = BLOCKRUN_MODELS.find(
    (m) => m.id === modelId || m.id === modelId.replace("aloe/", ""),
  );
...
export function supportsToolCalling(modelId: string): boolean {
  const normalized = modelId.replace("aloe/", "");
