...
describe("sanitizeHeaderValue", () => {
  ...
  it("percent-encodes Cyrillic so Node accepts the header value", () => {
    ...
    expect(() => validateHeaderValue("x-aloe-reasoning", sanitized)).not.toThrow();
  ...
  it("produces a valid header value for the real Russian-prompt repro", () => {
    ...
    // string used to go raw into the x-aloe-reasoning header.
    const prompt =
    ...
    expect(() => validateHeaderValue("x-aloe-reasoning", reasoning)).toThrow();
...
describe("debugHeadersEnabledFromEnv", () => {
  ...
  it.each(["0", "false", "off", "FALSE", "Off"])("is disabled by ALOE_DEBUG_HEADERS=%s", (v) => {
    expect(debugHeadersEnabledFromEnv({ ALOE_DEBUG_HEADERS: v })).toBe(false);
  });

  it("treats other values as enabled", () => {
    expect(debugHeadersEnabledFromEnv({ ALOE_DEBUG_HEADERS: "1" })).toBe(true);
    expect(debugHeadersEnabledFromEnv({ ALOE_DEBUG_HEADERS: "on" })).toBe(true);
  });
});
