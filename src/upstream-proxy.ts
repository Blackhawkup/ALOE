 ...
 * TUN mode): their curl tests succeed via the proxy while ALOE
 * connects directly and hits resets/throttling on the direct route.
 *
 * Usage:
 *   BLOCKRUN_UPSTREAM_PROXY=socks5://127.0.0.1:1080 aloe start
 *   BLOCKRUN_UPSTREAM_PROXY=http://127.0.0.1:8080 aloe start
 *   HTTPS_PROXY=http://127.0.0.1:7890 aloe start
 */
...
export async function applyUpstreamProxy(
  ...
  try {
    ...
    console.warn(`[ALOE] Invalid BLOCKRUN_UPSTREAM_PROXY URL: ${url} — skipping proxy setup`);
    return undefined;
  }
  ...
  try {
    if (scheme === "socks5:" || scheme === "socks4:") {
      ...
      console.warn(
        `[ALOE] Unsupported proxy scheme "${scheme}" in BLOCKRUN_UPSTREAM_PROXY — use http:// or socks5://`,
      );
      return undefined;
    }
  } catch (err) {
    console.warn(
      `[ALOE] Failed to configure upstream proxy "${url}": ${err instanceof Error ? err.message : err}`,
    );
    return undefined;
  }
...
async function applyEnvProxyFallback(
  ...
  try {
    ...
    console.warn(`[ALOE] Invalid proxy URL in environment: ${url} — skipping proxy setup`);
    return undefined;
  }

  if (parsed.protocol === "socks5:" || parsed.protocol === "socks4:") {
    console.warn(
      `[ALOE] SOCKS proxy found in environment but only applied when explicit — set BLOCKRUN_UPSTREAM_PROXY=${url} to route upstream traffic through it`,
    );
    return undefined;
  }
  ...
  if (parsed.protocol !== "http:" && parsed.protocol !== "https:") {
    console.warn(
      `[ALOE] Unsupported proxy scheme "${parsed.protocol}" in environment — use http:// or set BLOCKRUN_UPSTREAM_PROXY`,
    );
    return undefined;
  }
  ...
  try {
    ...
    console.warn(
      `[ALOE] Failed to configure env proxy "${url}": ${err instanceof Error ? err.message : err}`,
    );
    return undefined;
  }
