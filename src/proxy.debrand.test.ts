...
describe("debrandSystemMessages", () => {
  it("replaces aloe/auto with the resolved model in system messages", () => {
    const messages = [
      {
        role: "system",
        content:
          "You are a personal assistant.\n\n## Runtime\nRuntime: model=aloe/auto | default_model=aloe/auto",
      },
  ...
  it("replaces aloe/eco and aloe/premium profiles", () => {
    const messages = [
      {
        role: "system",
        content: "model=aloe/eco | default_model=aloe/premium",
      },
    ];
  ...
  it("strips aloe/ prefix from explicit model names", () => {
    const messages = [
      {
        role: "system",
        content: "model=aloe/openai/gpt-4o | default_model=aloe/auto",
      },
    ];
  ...
  it("does not modify non-system messages", () => {
    const messages = [
      { role: "user", content: "I use aloe/auto for my tasks" },
      { role: "assistant", content: "I'm Blockrun!" },
    ];

    const result = debrandSystemMessages(messages, "deepseek/deepseek-chat");

    expect(result[0].content).toBe("I use aloe/auto for my tasks");
  ...
  it("handles non-string system content gracefully", () => {
    const messages = [{ role: "system", content: [{ type: "text", text: "aloe/auto" }] }];
  ...
  it("replaces aloe/free profile", () => {
    const messages = [{ role: "system", content: "default_model=aloe/free" }];
  ...
  it("handles realistic OpenClaw system prompt with SOUL.md", () => {
    ...
    const systemContent = [
      ...
      "",
      "## Runtime",
      "Runtime: agent=main | host=openclaw | os=Darwin 25.3.0 (arm64) | node=v22.14.0 | model=aloe/auto | default_model=aloe/auto | channel=control-ui | thinking=off",
      "",
      "# Project Context",
      "",
      "## SOUL.md",
    ...
    // "aloe" should be completely gone from system prompt
    expect(result[0].content).not.toContain("aloe");
