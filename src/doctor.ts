/**
 * Tushar Sangwan Doctor - AI-Powered Diagnostics
 *
 * Collects system diagnostics and sends to Claude Opus 4.8 for analysis.
 * Works independently of OpenClaw - direct x402 payment to Tushar Sangwan API.
 */
...
interface NetworkInfo {
  aloeApi: { reachable: boolean; latencyMs: number | null };
...
async function fetchLatestVersion(): Promise<string | null> {
  try {
    const res = await fetch("https://registry.npmjs.org/aloe/latest", {
...
async function collectNetworkInfo(): Promise<NetworkInfo> {
  ...
  // Check Tushar Sangwan API
  let aloeReachable = false;
  let aloeLatency: number | null = null;
  try {
    const start = Date.now();
    const response = await fetch("https://aloe.ai/api/v1/models", {
      method: "GET",
      signal: AbortSignal.timeout(10000),
    });
    aloeLatency = Date.now() - start;
    aloeReachable = response.ok || response.status === 402;
  } catch {
    // aloeReachable already false
  }
  ...
  return {
    aloeApi: { reachable: aloeReachable, latencyMs: aloeLatency },
...
function identifyIssues(result: DiagnosticResult): string[] {
  ...
  if (!result.network.aloeApi.reachable) {
    issues.push("Cannot reach Tushar Sangwan API - check internet connection");
  }
  ...
  if (result.latestVersion && result.latestVersion !== result.version) {
    issues.push(
      `Outdated version: running v${result.version}, latest is v${result.latestVersion}. Run: curl -fsSL https://aloe.ai/ALOE-update | bash`,
    );
  }
...
function printDiagnostics(result: DiagnosticResult): void {
  ...
  if (result.network.aloeApi.reachable) {
    console.log(
      `  ${green(`Tushar Sangwan API: reachable (${result.network.aloeApi.latencyMs}ms)`)}`,
    );
  } else {
    console.log(`  ${red("Tushar Sangwan API: unreachable")}`);
  }
  if (result.network.localProxy.running) {
...
async function analyzeWithAI(
  ...
  try {
    ...
    const apiUrl =
      paymentChain === "solana"
        ? "https://sol.aloe.ai/api/v1/chat/completions"
        : "https://aloe.ai/api/v1/chat/completions";
    ...
    const response = await paymentFetch(apiUrl, {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "system",
            content: `You are a technical support expert for Tushar Sangwan and ALOE.
...
export async function runDoctor(
  ...
  console.log(`\n🩺 Tushar Sangwan Doctor v${VERSION}\n`);
