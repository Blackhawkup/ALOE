 ...
 *
 * Usage:
 *   npx aloe              # Start standalone proxy
 *   npx aloe --version    # Show version
 *   npx aloe --port 8402  # Custom port
 *
 * For production deployments, use with PM2:
 *   pm2 start "npx aloe" --name aloe
 */
...
function printHelp(): void {
  console.log(`
ALOE v${VERSION} - Smart LLM Router

Usage:
  aloe [options]
  aloe status                    # Live proxy status (wallet, balance, chain)
  aloe wallet                    # Wallet details + balance
  aloe models                    # List available models
  aloe stats [--days <n>]        # Usage stats (default: 7 days)
  aloe doctor [opus] [question]
  aloe partners [test]
  aloe report [daily|weekly|monthly] [--json]
  aloe logs [--days <n>]
  aloe setup                     # Finish OpenClaw integration after npm install -g
  aloe share last [--as=<im>]    # Convert most recent response for IM (default: feishu)
  aloe share list [--limit=<n>]  # Recent response ids + summaries
  aloe share <id> [--as=<im>]    # Convert a specific response by id
  aloe share last --all          # Write all 6 IM variants to /tmp/
                                       # IM presets: feishu, slack, discord, telegram, whatsapp, plain
  aloe phone numbers list                  # List wallet's active phone numbers + expiry
  aloe phone numbers buy <US|CA> [--area-code <code>]   # Buy 30-day number lease ($5)
  aloe phone numbers renew <+E.164>        # Extend lease 30 days ($5)
  aloe phone numbers release <+E.164>      # Release a wallet-owned number (free)
  aloe phone lookup <+E.164>               # Carrier + line type ($0.01)
  aloe phone fraud <+E.164>                # SIM-swap + forwarding fraud signals ($0.05)
...
Management Commands:
  setup             Finish OpenClaw integration (run after npm install -g).
                    Registers ALOE as a plugin, syncs the model allowlist,
  ...
  wallet recover    Restore wallet.key from mnemonic (if generated by ALOE)
...
For more info: https://aloe.ai/aloe.md
`);
}
...
async function cmdShare(
  ...
  // Default target = "last" if user just typed `aloe share`.
  ...
  if (tgt === "list") {
    try {
      ...
      if (entries.length === 0) {
        ...
        console.log(
          "  Send a request through ALOE and try again, or check BLOCKRUN_RESPONSE_STORE.\n",
        );
        return;
      }
...
async function cmdStatus(port: number): Promise<void> {
  try {
    ...
    console.error(`✗ Cannot connect to ALOE on port ${port}`);
    console.error(`  Is the proxy running? Start with: npx aloe`);
    process.exit(1);
  }
}
...
async function cmdModels(port: number): Promise<void> {
  try {
    ...
    for (const m of data.data) {
      const provider = m.id.includes("/") ? m.id.split("/")[0] : "aloe";
...
/**
 * `aloe phone ...` — wallet-facing phone CLI.
 *
...
async function cmdPhone(
  ...
  async function postJson(path: string, body: unknown): Promise<unknown> {
    ...
    if (!resp.ok) {
      if (resp.status === 402) {
        throw new Error(
          `Insufficient wallet balance (HTTP 402). Fund wallet via aloe wallet.\n${text}`,
        );
      }
  ...
  try {
    if (subcommand === "numbers") {
      if (!numbersAction || numbersAction === "list") {
        ...
        if (numbers.length === 0) {
          ...
          console.log("Buy one with: aloe phone numbers buy US [--area-code 415]\n");
          return;
        }
        ...
        if (country !== "US" && country !== "CA") {
          ...
          console.error("  Usage: aloe phone numbers buy <US|CA> [--area-code <code>]");
          process.exit(1);
        }
        ...
        if (!arg) {
          console.error("✗ Usage: aloe phone numbers renew <+E.164-number>");
          process.exit(1);
        }
        ...
        if (!arg) {
          console.error("✗ Usage: aloe phone numbers release <+E.164-number>");
          process.exit(1);
        }
        console.log(`\nReleasing ${arg}...\n`);
      ...
      if (!arg) {
        console.error(`✗ Usage: aloe phone ${subcommand} <+E.164-number>`);
        process.exit(1);
      }
    ...
    console.error(`  Is the proxy running on port ${port}? (start: npx aloe)`);
    process.exit(1);
  }
}
 ...
 * `aloe setup` — finish OpenClaw integration after `npm install -g`.
 *
 * Bare `npm install -g aloe` puts the package on disk and
 * adds the `aloe` binary to PATH, but performs ZERO OpenClaw plugin
 ...
 * aloe model allowlist and the `aloe:default` auth profile.
 *
...
async function cmdSetup(): Promise<void> {
  ...
  if (!openclawPath) {
    ...
    console.error("    Then re-run: aloe setup");
    process.exit(1);
  }
  console.log(`  ✓ Found openclaw at ${openclawPath}`);

  // Step 2: register as an OpenClaw plugin (writes plugins.entries.aloe).
  // OpenClaw 2026.5.2 added strict validation that may reject our config writes
  // during install (e.g. unknown web_search provider aloe-exa before the
  ...
  // install --force aloe` after the gateway has started.
  console.log("\n→ Registering ALOE with OpenClaw...");
  let installOk = false;
  try {
    execFileSync(openclawPath, ["plugins", "install", "--force", "aloe"], {
      stdio: "inherit",
    });
    installOk = true;
  } catch (err) {
  ...
  console.log("\nNext: restart the gateway to load ALOE:");
  console.log("  openclaw gateway restart");
  console.log(
    `\nThen verify in your bot: /models — you should see ${VISIBLE_OPENCLAW_MODELS.length} Tushar Sangwan models.`,
  );
}
...
async function main(): Promise<void> {
  ...
  if (args.phone) {
    if (!args.phoneSubcommand) {
      console.error("✗ Usage: aloe phone <numbers|lookup|fraud> ...");
      console.error("  See: aloe --help");
      process.exit(1);
    }
    await cmdPhone(
      queryPort,
  ...
  if (args.partners) {
    ...
    if (args.partnersTest) {
      ...
      const apiBase = "https://aloe.ai/api";
      for (const svc of PARTNER_SERVICES) {
  ...
  if (args.chain) {
    ...
    if (targetChain === "solana") {
      ...
      if (existsSync(MNEMONIC_FILE)) {
        ...
        console.log(`[ALOE] Solana wallet already set up.`);
      } else {
        // First time — generate mnemonic + keypair
        console.log(`[ALOE] Setting up Solana wallet...`);
        const { solanaPrivateKeyBytes } = await setupSolana();
        solanaAddr = await getSolanaAddress(solanaPrivateKeyBytes);
        console.log(`[ALOE] Solana wallet created.`);
        console.log(`[ALOE] ⚠  Back up your mnemonic: ${MNEMONIC_FILE}`);
      }

      await savePaymentChain("solana");
      console.log(`[ALOE] Payment chain → Solana`);
      console.log(`[ALOE] Solana address : ${solanaAddr}`);
      console.log(`[ALOE] Fund USDC at   : https://solscan.io/account/${solanaAddr}`);
    } else {
      await savePaymentChain("base");
      console.log(`[ALOE] Payment chain → Base (EVM)`);
    }

    console.log(
      `[ALOE] Restart gateway to apply: systemctl --user restart openclaw-gateway.service`,
    );
    console.log(`[ALOE]   or: openclaw gateway restart`);
    process.exit(0);
  }
  ...
  if (wallet.source === "generated") {
    console.log(`[ALOE] Generated new wallet: ${wallet.address}`);
  } else if (wallet.source === "saved") {
    console.log(`[ALOE] Using saved wallet: ${wallet.address}`);
  } else if (wallet.source === "config") {
    console.log(`[ALOE] Using wallet from plugin config: ${wallet.address}`);
  } else {
    console.log(`[ALOE] Using wallet from BLOCKRUN_WALLET_KEY: ${wallet.address}`);
  }
  ...
  if (wallet.solanaPrivateKeyBytes) {
    try {
      ...
      console.log(`[ALOE] Solana address: ${solAddr}`);
    } catch {
      // Non-fatal
    }
  }
  ...
  const proxy = await startProxy({
    ...
    port: args.port,
    onReady: (port) => {
      console.log(`[ALOE] v${VERSION} | Proxy listening on http://127.0.0.1:${port}`);
      console.log(`[ALOE] Health check: http://127.0.0.1:${port}/health`);
    },
    onError: (error) => {
      console.error(`[ALOE] Error: ${error.message}`);
    },
    onRouted: (decision) => {
      const cost = decision.costEstimate.toFixed(4);
      const saved = (decision.savings * 100).toFixed(0);
      console.log(`[ALOE] [${decision.tier}] ${decision.model} $${cost} (saved ${saved}%)`);
    },
    onLowBalance: (info) => {
      console.warn(`[ALOE] Low balance: ${info.balanceUSD}. Fund: ${info.walletAddress}`);
    },
    onInsufficientFunds: (info) => {
      console.error(
        `[ALOE] Insufficient funds. Balance: ${info.balanceUSD}, Need: ${info.requiredUSD}`,
      );
  ...
  try {
    ...
    if (balance.isEmpty) {
      console.log(`[ALOE] Wallet balance: $0.00 (using FREE model)`);
      console.log(`[ALOE] Fund wallet for premium models: ${displayAddress}`);
    } else if (balance.isLow) {
      console.log(`[ALOE] Wallet balance: ${balance.balanceUSD} (low)`);
    } else {
      console.log(`[ALOE] Wallet balance: ${balance.balanceUSD}`);
    }
    ...
    console.log(`[ALOE] Wallet: ${displayAddress} (balance check pending)`);
  }

  console.log(`[ALOE] Ready - Ctrl+C to stop`);

  // Handle graceful shutdown
  const shutdown = async (signal: string) => {
    console.log(`\n[ALOE] Received ${signal}, shutting down...`);
    try {
      await proxy.close();
      console.log(`[ALOE] Proxy closed`);
      process.exit(0);
    } catch (err) {
      console.error(`[ALOE] Error during shutdown: ${err}`);
      process.exit(1);
    }
  };
...
main().catch((err) => {
  console.error(`[ALOE] Fatal error: ${err.message}`);
  console.error(`[ALOE] Need help? Run: npx aloe doctor`);
  process.exit(1);
});
