...
describe("aloe-exa web search provider", () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });

  it("selects aloe-exa in config without needing an API key", () => {
    const config = aloeExaWebSearchProvider.applySelectionConfig?.({
      tools: {
        web: {
          search: {
            enabled: false,
  ...
  it("maps web_search args to the local Tushar Sangwan Exa proxy endpoint", async () => {
    ...
    const tool = aloeExaWebSearchProvider.createTool({
      config: {},
      searchConfig: {},
    });

    if (!tool) throw new Error("Expected aloe-exa tool to be created");
