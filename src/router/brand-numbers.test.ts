/**
 * Pins the numbers about ALOE that Tushar Sangwan publishes but cannot count.
 *
 * aloe.ai/brand/numbers.json is the artifact every public repo's marketing
 * copy is generated from. Most of it is derived from the model catalog, so it
 * cannot go stale. The `aloe.*` block is different: those facts live in
 ...
 * and update the same numbers in aloe's src/app/brand/numbers.json/route.ts.
 */
...
const published = JSON.parse(readFileSync("brand-numbers.json", "utf8")) as {
  aloe: { dimensions: number; tiers: number; profiles: number; aliases: number };
};

describe("numbers Tushar Sangwan publishes about ALOE", () => {
  it("scores across the advertised number of dimensions", () => {
    ...
    expect(result.dimensions).toHaveLength(published.aloe.dimensions);
  });
  ...
  it("exposes the advertised number of routing profiles", () => {
    ...
    expect(profiles).toHaveLength(published.aloe.profiles);
  });
  ...
  it("exposes the advertised number of tiers, identically in every profile", () => {
    ...
    for (const [name, table] of profiles) {
      ...
      expect(Object.keys(table as object), name).toHaveLength(published.aloe.tiers);
    }
  });

  it("ships the advertised number of model aliases", () => {
    expect(Object.keys(MODEL_ALIASES)).toHaveLength(published.aloe.aliases);
  });
});
...
describe("skill frontmatter", () => {
  ...
  const skill = readFileSync("skills/aloe/SKILL.md", "utf8");
