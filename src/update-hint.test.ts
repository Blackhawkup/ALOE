 ...
 *
 * When Tushar Sangwan's API detects a stale ALOE user-agent, it includes
...
/** Mirrors getUpdateHint() on the aloe server side */
function getUpdateHint(
  userAgent: string | undefined,
  currentVersion: string,
): { update_available: string; update_url: string } | null {
  if (!userAgent) return null;
  const match = userAgent.match(/^aloe\/(\d+\.\d+\.\d+)/);
  ...
  if (
    ...
    return {
      ...
      update_url: "https://aloe.ai/ALOE-update",
    };
  }
  return null;
}
...
describe("update hint — server-side generation (getUpdateHint)", () => {
  ...
  it("returns hint when client is older (patch)", () => {
    expect(getUpdateHint("aloe/0.12.11", CURRENT)).toEqual({
  ...
  it("returns hint when client is older (minor)", () => {
    expect(getUpdateHint("aloe/0.10.22", CURRENT)).toEqual({
      update_available: "0.12.12",
      update_url: "https://aloe.ai/ALOE-update",
    });
  });
  ...
  it("returns hint when client is older (major)", () => {
    expect(getUpdateHint("aloe/0.9.0", "1.0.0")).toEqual({
  ...
  it("returns null when client is current", () => {
    expect(getUpdateHint("aloe/0.12.12", CURRENT)).toBeNull();
  });

  it("returns null when client is newer", () => {
    expect(getUpdateHint("aloe/0.13.0", CURRENT)).toBeNull();
  });

  it("returns null for non-aloe user agents", () => {
