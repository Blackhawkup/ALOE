 ...
 * `aloe share last` can recover them across proxy restarts.
 *
 * Storage format: append-only daily JSONL, mirroring src/logger.ts.
 * Path: ~/.openclaw/aloe/responses/responses-YYYY-MM-DD.jsonl
 *
...
const STORE_DIR = join(homedir(), ".openclaw", "aloe", "responses");
let dirReady = false;
