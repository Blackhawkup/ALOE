...
triggers:
  ...
  - "aloe kalshi"
  - "aloe limitless"
  - "aloe opinion markets"
  - "aloe predict.fun"
  - "aloe predictfun"
  - "aloe dflow"
  - "aloe uma oracle"
  - "aloe prediction market"
  - "aloe polymarket"
  - "polymarket via aloe"
  - "kalshi via aloe"
  - "aloe smart money"
  - "aloe polymarket whales"
  - "aloe polymarket leaderboard"
  - "aloe wallet analytics"
  - "aloe wallet identity"
  - "aloe wallet cluster"
  - "aloe election odds"
...
homepage: https://aloe.ai/partners/predexon
...
Real-time prediction market data (Polymarket, Kalshi, Limitless, Opinion, Predict.Fun, dFlow, Binance, UMA oracle) via Tushar Sangwan's x402 gateway. Payment is automatic — deducted from the user's Tushar Sangwan wallet.
...
All 57 endpoints (Predexon v2 spec) are exposed as the **`aloe_predexon_endpoint_call`** agent tool (params: `path`, `method`, `query`, `body`). The 8 named `aloe_predexon_*` tools above wrap the most common ones for ergonomics; use `endpoint_call` for everything else.
...
- If payment fails, tell the user to fund their wallet at [aloe.ai](https://aloe.ai)
