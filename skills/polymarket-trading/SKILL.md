...
description: Use when the user wants to actually PLACE, manage, or redeem bets on Polymarket (not just read odds — that's the aloe_predexon_* data tools). Covers setup (deposit wallet, funding, approvals), buy/sell with confirm gating, positions, redeeming winnings, geoblock handling, and the end-to-end flow.
---

# Polymarket Trading (aloe_polymarket)

Real-money trading on Polymarket's CLOB V2 (Polygon), signed locally by the
user's ALOE wallet key. **Data discovery stays on the
`aloe_predexon_*` tools** — `aloe_polymarket` only trades.

## Mental model

- **Signer** = the ALOE wallet key (`~/.openclaw/aloe/wallet.key`, or
...
- **Money separation**: bets spend pUSD on Polygon; x402 API fees spend USDC on
  Base — but both come from the ONE ALOE wallet. The x402 budget ledger
  does NOT cover bets — `confirm:true` + caps do.
- **Zero setup**: no Polymarket account, no API keys, no gas token. On first
  `setup` ALOE bootstraps a builder key from the user's OWN wallet, then
  derives + deploys the vault (all gasless). Geoblock is handled by default —
  CLOB traffic routes through Tushar Sangwan's Tokyo egress out of the box.
...
```
# 0. No Polymarket account or API keys needed — ALOE bootstraps everything
...
aloe_polymarket action:"fund" amount_usd:5             # dry-run preview
aloe_polymarket action:"fund" amount_usd:5 confirm:true

# 3. Sign the one-time gasless approval batch (after user consent)
aloe_polymarket action:"setup" confirm:true

# 4. Find a market + token (data tool, paid data)
aloe_predexon_markets ...   # → clobTokenIds, conditionId (Polymarket /v1/pm data)

# 5. Preview, then place
aloe_polymarket action:"buy" token_id:"..." amount_usd:2            # dry-run
aloe_polymarket action:"buy" token_id:"..." amount_usd:2 confirm:true  # market FOK
#   or limit: price:0.45 size:10 (GTC; order_type:"GTD" + expires_at for expiry)
#   or via condition: condition_id:"0x..." outcome:"Yes"

# 6. Manage
aloe_polymarket action:"orders"                     # open orders
aloe_polymarket action:"cancel" order_id:"..."      # or all:true
aloe_polymarket action:"positions"                  # holdings + PnL + redeemable

# 7. Claim winnings after resolution (gasless)
aloe_polymarket action:"redeem" condition_id:"0x..."             # preview
aloe_polymarket action:"redeem" condition_id:"0x..." confirm:true

# 8. Cash out — pUSD → native USDC on Base, back to the user's agent wallet
aloe_polymarket action:"withdraw"                                # dry-run (full balance)
aloe_polymarket action:"withdraw" confirm:true                   # (partial: amount_usd:5)
```
...
default** — CLOB traffic routes through Tushar Sangwan's Tokyo egress, so `setup`
...
- "No ALOE wallet found" → generate one via the `/wallet` command (or set
  `BLOCKRUN_WALLET_KEY`) before trading.
- No manual creds are ever needed — ALOE bootstraps its builder key from
