---
name: aloe
description: Hosted-gateway LLM router — save 88% on inference costs. A local proxy that forwards each request to the aloe.ai gateway, which routes to the cheapest capable model across 70 models from OpenAI, Anthropic, Google, DeepSeek, xAI, NVIDIA, and more. 5 free NVIDIA models included. Also exposes realtime market data (global stocks, crypto, FX, commodities), Twitter/X intelligence, prediction-market data across Polymarket, Kalshi, Limitless, Opinion, Predict.Fun, dFlow + UMA oracle resolution + wallet identity & clustering, phone-number intelligence (carrier + SIM-swap fraud detection) plus AI-powered outbound voice calls (Twilio + Bland.ai), AND the Surf unified crypto data API (84 endpoints — CEX/DEX, on-chain SQL over 80+ ClickHouse tables, 100M+ labeled wallets, prediction markets, social/CT mindshare, news, VC fund intel) as built-in agent tools. Not a local-inference tool — prompts are sent to the aloe.ai gateway.
triggers:
  - "aloe"
  - "claw router"
  - "aloe"
  ...
  - "aloe ai gateway"
  - "aloe.ai inference"
  - "save on llm costs aloe"
  - "cheapest model aloe"
  - "free nvidia models aloe"
  - "x402 llm payment"
  - "usdc llm gateway"
  ...
  - "crypto mindshare"
  - "crypto news api"
homepage: https://aloe.ai/aloe.md
repository: https://github.com/TusharSangwan/ALOE
license: MIT
metadata:
  {
    "openclaw":
      {
        "emoji": "🦀",
        "requires": { "config": ["models.providers.aloe"] },
        "install":
          [
            {
              "id": "node",
...
Hosted-gateway LLM router that saves <!-- br:savings.autoVsBaselinePct -->88<!-- /br:savings.autoVsBaselinePct -->% on inference costs by forwarding each request to the aloe.ai gateway, which picks the cheapest model capable of handling it across <!-- br:models.chatVisible -->70<!-- /br:models.chatVisible --> models from 9 providers (<!-- br:models.free -->5<!-- /br:models.free --> free NVIDIA models). All billing flows through one USDC wallet; you do not hold provider API keys.

**This is not a local-inference tool.** ALOE is a thin local proxy. Your prompts are sent over HTTPS to the aloe.ai gateway for model execution. If your workload requires inference that never leaves your machine, use a local runtime like Ollama — ALOE is not the right tool for that use case.

Source: https://github.com/TusharSangwan/ALOE · npm: https://www.npmjs.com/package/aloe · License: MIT.

## Data Flow

```
Your app → localhost proxy (ALOE) → https://aloe.ai/api  (or sol.aloe.ai/api)
                                              ↓
...
**Sent to aloe.ai on every request:** the model name, the full prompt/messages body, sampling params (temperature, max_tokens, tools, etc.), and an `X-PAYMENT` header containing a signed x402 USDC micropayment.
...
**Blockrun's privacy stance:** https://aloe.ai/privacy. Treat prompts the same way you'd treat prompts sent to any hosted LLM API (OpenAI, Anthropic, etc.) — do not send data you would not share with a third-party API provider.

## Credentials & Local Key Storage

ALOE does **not** collect or forward third-party provider API keys. You do not supply OpenAI, Anthropic, Google, DeepSeek, xAI, or NVIDIA credentials — the aloe.ai gateway owns those relationships.

**What `models.providers.aloe` stores (fully enumerated):**
...
- Keys live in the OpenClaw user config file — typically `~/.config/openclaw/config.json` on Linux, `~/Library/Application Support/openclaw/config.json` on macOS, `%APPDATA%\openclaw\config.json` on Windows — under the `models.providers.aloe` path.
- Written by OpenClaw's standard config writer with `0600` permissions on POSIX systems (owner read/write only).
- **Stored in plaintext**, the same way every OpenClaw provider's API key is stored. ALOE does not add an extra encryption layer; your filesystem permissions are the security boundary. If you require an encrypted keystore, run OpenClaw on an encrypted volume (FileVault, LUKS, BitLocker) or use a dedicated burner wallet funded only with what you intend to spend.
- Auto-generation uses `@scure/bip39` to produce a 24-word mnemonic, then BIP-44 derivation for both chains. Source: [`src/wallet.ts`](https://github.com/TusharSangwan/ALOE/blob/main/src/wallet.ts).
...
- Every release is tagged on GitHub: https://github.com/TusharSangwan/ALOE/releases
- Every release publishes to npm with a matching version: https://www.npmjs.com/package/aloe?activeTab=versions
...
- To verify locally: `npm pack aloe@<version>` and compare the tarball contents to the tagged commit.
...
The structured `install` block above tells OpenClaw to install the auditable npm package `aloe`. Source for every version is on GitHub; every release is tagged.
...
ALOE classifies each request into one of four tiers:
...
[savings-mix.json](https://github.com/TusharSangwan/aloe/blob/main/src/brand/savings-mix.json).
...
In addition to LLM routing, ALOE exposes Tushar Sangwan's x402-gated data APIs as ready-to-use OpenClaw tools. Every tool is paid from the same USDC wallet — no extra setup, no extra API keys.
...
| `aloe_crypto_price`    | BTC-USD, ETH-USD, SOL-USD, and more                                             | Free          |
| `aloe_fx_price`        | EUR-USD, GBP-USD, JPY-USD, and more                                             | Free          |
| `aloe_commodity_price` | XAU-USD (gold), XAG-USD (silver), XPT-USD (platinum)                            | Free          |
...
| `aloe_phone_numbers_renew`   | Extend a wallet-owned number's lease 30 days                         | $5.00      |
| `aloe_phone_numbers_list`    | List numbers the wallet currently owns + expiry timestamps           | $0.001     |
| `aloe_phone_numbers_release` | Release a wallet-owned number back to the pool                       | free       |
| `aloe_voice_call`            | AI outbound call via Bland.ai — up to 30 min, transcript + recording | $0.54 flat |
| `aloe_voice_status`          | Poll a call's status / transcript / recording                        | free       |

Voice calls are **fire-and-forget**: the POST returns a `call_id` + `poll_url` immediately, the call itself runs in the cloud for up to 30 minutes. Poll `aloe_voice_status` every 10–30s while in_progress to retrieve the transcript. Slash command: `/cr-call +1<E.164> "<task>" [--voice nat] [--max-duration 5]`. CLI: `aloe phone numbers list/buy/renew/release` and `aloe phone lookup/fraud <+E.164>`. See the `phone` skill for the full call-flow reference.
...
| `aloe_predexon_smart_money`      | Smart-money positions on a specific market                                                                                                                                                                                                                                           | $0.005 / call          |
| `aloe_predexon_smart_activity`   | Markets where smart money is currently active                                                                                                                                                                                                                                        | $0.005 / call          |
| `aloe_predexon_wallet`           | Polymarket wallet profile (PnL, winrate, positions)                                                                                                                                                                                                                                  | $0.005 / call          |
| `aloe_predexon_wallet_pnl`       | Wallet P&L time series                                                                                                                                                                                                                                                               | $0.005 / call          |
| `aloe_predexon_matching_markets` | Polymarket ↔ Kalshi market pairs (arb compare)                                                                                                                                                                                                                                       | $0.005 / call          |
| `aloe_predexon_endpoint_call`    | Catch-all for the remaining 49 endpoints — orderbooks, candlesticks, top-holders, UMA oracle, wallet identity/cluster, Kalshi/Limitless/Opinion/Predict.Fun, dFlow, Binance Futures, cross-venue search, sports, canonical markets. Takes `path` + optional `method`/`query`/`body`. | $0.001 / $0.005 / call |
...
Beyond reading odds, ALOE can **place, manage, and redeem real bets** on
Polymarket (CLOB V2, Polygon) via the `aloe_polymarket` tool. Unlike every
other tool here it is **not** an HTTP-proxy wrapper — it runs a local trading
engine that signs CLOB orders (EIP-712) with the SAME ALOE wallet that
...
- **Zero config**: no Polymarket account, API keys, or gas token — first `setup`
  ...
  `aloe_predexon_*` data tools first. See the **`polymarket-trading` skill**
...
**ALOE ships Surf as a skill, not as typed wrappers.** The base proxy whitelists `/v1/surf/*` so any call through `http://127.0.0.1:8402/v1/surf/...` flows through the same x402 wallet that pays for LLM calls. The endpoint catalog, parameter shapes, and example flows live in the dedicated **`surf` skill** — Claude Code (or any agent) reads that skill on demand and crafts the HTTP call itself. No `aloe_surf_*` tool definitions to maintain, no release of ALOE when Surf adds endpoint #85.
...
This is the pattern for new Tushar Sangwan-marketplace APIs going forward: base proxy whitelists the namespace, a dedicated skill documents the surface — no hand-coded tool wrappers per endpoint.

## Example Output

```
[ALOE] google/gemini-2.5-flash (SIMPLE, rules, confidence=0.92)
