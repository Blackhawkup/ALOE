---
name: release
description: Use this skill for EVERY ALOE release. Enforces the full checklist — version sync, CHANGELOG, build, tests, npm publish, git tag, GitHub release. No step can be skipped.
triggers:
  - "release aloe"
  - "ship aloe"
  - "publish aloe"
...
updating a `CURRENT_ALOE_VERSION` constant in aloe's
`src/app/api/v1/chat/completions/route.ts`. That constant has been replaced:
the server now `fetch`-es `https://registry.npmjs.org/aloe/latest`
...
```typescript
// aloe/src/app/api/v1/chat/completions/route.ts
let latestALOEVersion: string | null = process.env.ALOE_CURRENT_VERSION || null;
fetch("https://registry.npmjs.org/aloe/latest", {
  signal: AbortSignal.timeout(5000),
})
  .then((r) => r.json())
  .then((data) => {
    if (data.version) latestALOEVersion = data.version;
  })
  .catch(() => {
...
- The `npm publish` in Step 11 IS the entire "sync aloe" action. After that
  step lands, the next aloe server restart will fetch and surface the new
  version. No manual constant edit. No separate PR in the aloe repo.
- `ALOE_CURRENT_VERSION` env var is the cold-start fallback (used only
...
- Verification of the user-facing update nudge happens in Step 12 via
  `npm view aloe version` (the same registry endpoint
  aloe's server hits).
...
No companion commit in the aloe repo is needed — the server picks up
...
Verify the release on GitHub: https://github.com/TusharSangwan/ALOE/releases
...
Verify: https://npmjs.com/package/aloe

Expected output: `+ aloe@{VERSION}`
...
# 3. npm package is live (this is also what aloe's server fetches —
#    matching version here means the user-facing update nudge will fire correctly)
npm view aloe version
