...
# ALOE Docker Installation Tests
...
test_fresh_install() {
    ...
    if command -v aloe &> /dev/null; then
        log_pass "ALOE installed successfully"
    else
        log_fail "ALOE command not found after installation"
        return 1
    fi

    VERSION=$(aloe --version 2>/dev/null || echo "")
    if [ -n "$VERSION" ]; then
...
test_uninstall() {
    ...
    if ! command -v aloe &> /dev/null; then
        log_pass "ALOE uninstalled successfully"
    else
        log_fail "ALOE command still available after uninstall"
        return 1
    fi
}
...
test_reinstall() {
    ...
    if command -v aloe &> /dev/null; then
        log_pass "ALOE reinstalled successfully"
    else
        log_fail "ALOE command not found after reinstall"
        return 1
    fi
}
...
test_openclaw_install() {
    ...
    PLUGIN_FILE="$HOME/.npm-global/lib/node_modules/aloe/openclaw.plugin.json"
...
test_upgrade() {
    ...
    npm uninstall -g aloe 2>/dev/null || true

    # Install old version
    npm install -g aloe@0.8.25

    OLD_VERSION=$(aloe --version 2>/dev/null || echo "")
    log_info "Installed version: $OLD_VERSION"

    # Upgrade to latest
    npm install -g aloe

    NEW_VERSION=$(aloe --version 2>/dev/null || echo "")
...
test_custom_wallet() {
    ...
    export ALOE_WALLET_PRIVATE_KEY="$CUSTOM_KEY"

    # Verify installation with custom key works
    if command -v aloe &> /dev/null; then
        log_pass "ALOE installed with custom wallet key"
    else
        log_fail "Installation failed with custom wallet key"
        return 1
    fi

    unset ALOE_WALLET_PRIVATE_KEY
}
...
test_package_files() {
    ...
    PKG_DIR="$HOME/.npm-global/lib/node_modules/aloe"
...
test_version_accuracy() {
    ...
    CLI_VERSION=$(aloe --version 2>/dev/null || echo "")
...
test_full_cleanup() {
    ...
    if ! command -v aloe &> /dev/null; then
        log_pass "ALOE command removed from PATH"
    else
        log_fail "ALOE command still in PATH"
        return 1
    fi
}
