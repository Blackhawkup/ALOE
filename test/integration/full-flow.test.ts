...
describe.skipIf(!process.env.BLOCKRUN_WALLET_KEY)("ALOE full-flow (funded wallet)", () => {
  beforeAll(async () => {
  ...
  it("chat completion with aloe/free returns valid response", async () => {
  ...
  it("chat completion with aloe/auto resolves to a model", async () => {
    const res = await fetch(`${getTestProxyUrl()}/v1/chat/completions`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
