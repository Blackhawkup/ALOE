 ...
 * against ALOE's built dist/ to catch regressions like process.env
...
/** Resolve ALOE dist dir — relative to this file, fall back to Docker path. */
function resolveALOEDist(): string {
  const local = resolve(__dirname, "../../dist");
  try {
    readdirSync(local);
    return local;
  } catch {
    return "/opt/aloe/dist";
  }
}
