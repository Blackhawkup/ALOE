...
describe("/debug command", () => {
  ...
  it("returns routing diagnostics for /debug with no prompt (non-streaming)", async () => {
    ...
    expect(body.model).toBe("aloe/debug");
    expect(body.id).toMatch(/^chatcmpl-debug-/);
    expect(body.choices).toHaveLength(1);
    expect(body.choices[0].finish_reason).toBe("stop");

    const content = body.choices[0].message.content;
    expect(content).toContain("ALOE Debug");
  ...
  it("returns SSE streaming response for /debug with stream: true", async () => {
    ...
    expect(text).toContain("ALOE Debug");
    expect(text).toContain("aloe/debug");
  });
  ...
  it("does not intercept normal messages that just contain 'debug'", async () => {
    ...
    // we do NOT immediately get the synthetic aloe/debug response.
    try {
      const res = await fetch(`${getTestProxyUrl()}/v1/chat/completions`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      // Whether it succeeds or fails upstream, the model should not be "aloe/debug"
      if (res.status === 200) {
        const body = (await res.json()) as { model: string };
        expect(body.model).not.toBe("aloe/debug");
      }
    } catch (err) {
