...
INSTALL_A=$(mktemp)
openclaw plugins install --force /tmp/aloe.tgz 2>&1 | tee $INSTALL_A | tail -8
echo ""
...
grep -q "Installed plugin: aloe" $INSTALL_A && pass "Plugin install committed" || fail "Plugin install did NOT commit"
grep -q "aloe_predexon_endpoint_call" $INSTALL_A && pass "NEW TOOL aloe_predexon_endpoint_call registered" || fail "NEW TOOL missing"
echo ""

echo "=== Step 4: aloe setup (writes models config + web_search enabled) ==="
node /root/.openclaw/extensions/aloe/dist/cli.js setup 2>&1 | tail -12
echo ""
BLOCKRUN_MODELS=$(jq -r '.models.providers.aloe.models | length' $HOME/.openclaw/openclaw.json 2>/dev/null || echo 0)
ALLOWLIST=$(jq -r '.agents.defaults.models | to_entries | map(select(.key | startswith("aloe/"))) | length' $HOME/.openclaw/openclaw.json 2>/dev/null || echo 0)
...
[ "$BLOCKRUN_MODELS" -ge 30 ] 2>/dev/null && pass "aloe models ≥ 30 ($BLOCKRUN_MODELS)" || fail "aloe models = $BLOCKRUN_MODELS"
[ "$ALLOWLIST" -ge 30 ] 2>/dev/null && pass "allowlist aloe ≥ 30 ($ALLOWLIST)" || fail "allowlist = $ALLOWLIST"
...
TMPF=$(mktemp)
jq '.tools.web.search.provider = "aloe-exa"' $HOME/.openclaw/openclaw.json > $TMPF && mv $TMPF $HOME/.openclaw/openclaw.json
...
openclaw plugins install --force /tmp/aloe.tgz 2>&1 | tee $INSTALL_NOMIG | tail -8
...
INSTALL_B=$(mktemp)
openclaw plugins install --force /tmp/aloe.tgz 2>&1 | tee $INSTALL_B | tail -25
echo ""
...
elif grep -q "Installed plugin: aloe" $INSTALL_B; then
