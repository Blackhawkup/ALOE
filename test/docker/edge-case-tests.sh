...
ALOE_PORT=8402
BLOCKRUN_API="https://api.aloe.ai/v1"
...
test_fresh_install() {
    ...
    npm install -g aloe 2>&1 | head -5

    if command -v aloe &> /dev/null; then
        log_pass "ALOE installed"
        aloe --version
    else
        log_fail "ALOE not in PATH"
        return 1
    fi
}
...
test_openclaw_plugin_install() {
    ...
    bash $(npm root -g)/aloe/scripts/reinstall.sh 2>&1 || {
        log_fail "Reinstall script failed"
        return 1
    }

    # Verify extension installed
    if [ -f "$HOME/.openclaw/extensions/aloe/dist/index.js" ]; then
...
test_aloe_prefix_stripping() {
    log_test "2.2" "aloe/ prefix stripping"

    # These models should all resolve correctly
    local models=(
        "aloe/anthropic/claude-sonnet-4.6"
        "aloe/openai/gpt-4o"
        "aloe/deepseek/deepseek-chat"
...
test_invalid_model_handling() {
    ...
    local response
    response=$(curl -s -X POST "http://localhost:$ALOE_PORT/v1/chat/completions" \
...
test_proxy_startup() {
    log_test "3.1" "Proxy startup on port $ALOE_PORT"

    # Kill any existing proxy
    lsof -ti :$ALOE_PORT | xargs kill -9 2>/dev/null || true
    sleep 1

    # Start proxy in background
    BLOCKRUN_WALLET_KEY="0x$(openssl rand -hex 32)" aloe &
    PROXY_PID=$!

    if wait_for_port $ALOE_PORT 10; then
        log_pass "Proxy started on port $ALOE_PORT (PID: $PROXY_PID)"
    else
...
test_proxy_health_check() {
    ...
    local response
    response=$(curl -s "http://localhost:$ALOE_PORT/health" 2>/dev/null || echo "failed")
...
test_proxy_models_endpoint() {
    ...
    local response
    response=$(curl -s "http://localhost:$ALOE_PORT/v1/models" 2>/dev/null || echo '{"error":"failed"}')
...
test_proxy_graceful_shutdown() {
    ...
    local pid=$(lsof -ti :$ALOE_PORT 2>/dev/null || echo "")

    if [ -n "$pid" ]; then
        kill $pid 2>/dev/null
        sleep 2

        if ! nc -z localhost $ALOE_PORT 2>/dev/null; then
...
test_port_conflict_handling() {
    ...
    # Try to start aloe (should fail or use different port)
    BLOCKRUN_WALLET_KEY="0x$(openssl rand -hex 32)" timeout 5 aloe 2>&1 | head -3 || true
...
test_wallet_generation() {
    ...
    rm -f ~/.aloe/wallet.json 2>/dev/null || true

    # Start proxy briefly to generate wallet
    BLOCKRUN_WALLET_KEY="" timeout 3 aloe 2>&1 | head -5 || true

    if [ -f ~/.aloe/wallet.json ]; then
        log_pass "Wallet auto-generated"
        jq -r '.address' ~/.aloe/wallet.json | head -1
    else
...
test_wallet_persistence() {
    ...
    get_wallet_from_health() {
        ...
        BLOCKRUN_WALLET_KEY="$key" aloe >/tmp/aloe-wallet-persistence.log 2>&1 &
        pid=$!
        ...
        response=$(curl -s "http://localhost:$ALOE_PORT/health" 2>/dev/null || echo "")
...
test_openclaw_config_injection() {
    ...
    if [ -f "$CONFIG_FILE" ]; then
        if jq -e '.models.providers.aloe' "$CONFIG_FILE" &>/dev/null; then
            log_pass "aloe provider configured in openclaw.json"
        else
            log_fail "aloe provider not found in config"
        fi
    else
...
test_openclaw_auth_profile() {
    ...
    if [ -f "$AUTH_FILE" ]; then
        if jq -e '.profiles["aloe:default"]' "$AUTH_FILE" &>/dev/null; then
            log_pass "aloe:default auth profile exists"
        else
            log_fail "aloe:default profile not found"
        fi
    else
...
test_openclaw_plugins_allow() {
    ...
    if [ -f "$CONFIG_FILE" ]; then
        if jq -e '.plugins.allow | index("aloe") or index("aloe")' "$CONFIG_FILE" &>/dev/null; then
            log_pass "aloe in plugins.allow"
        else
            log_fail "aloe not in plugins.allow"
        fi
    else
        log_skip "openclaw.json not found"
    fi
...
main() {
    echo ""
    echo -e "${CYAN}╔══════════════════════════════════════════════════════════════╗${NC}"
    echo -e "${CYAN}║       ALOE Edge Case Test Suite v1.0                   ║${NC}"
    ...
    test_aloe_prefix_stripping || true
