 ...
 * ALOE correctly falls back to the next model in the chain.
 *
 * Usage:
...
// Mock Tushar Sangwan API server
...
async function runTests() {
  ...
  // Test 6b: Delisted explicit model IDs should alias to a live replacement
  {
    ...
    const res = await fetch(`${proxy.baseUrl}/v1/chat/completions`, {
      ...
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        model: "aloe/xai/grok-code-fast-1",
  ...
  // This proves the projected-cost check, not just a trailing "already exceeded" check.
  {
    ...
    assert(
      res2.headers.get("X-ALOE-Cost-Cap-Exceeded") === "1",
      `X-ALOE-Cost-Cap-Exceeded header set: ${res2.headers.get("X-ALOE-Cost-Cap-Exceeded")}`,
    );
  ...
  // Uses aloe/auto (routing profile) so ALOE chose the model and can downgrade.
  // cap=$0.0015: first request succeeds ($0.001 minimum cost), second is downgraded ($0.0005 left).
  {
    ...
    // First request: aloe/auto routing profile, succeeds, accumulates cost ($0.001 min)
    ...
    assert(
      res2.headers.get("x-aloe-budget-downgrade") === "1",
      `x-aloe-budget-downgrade header set: ${res2.headers.get("x-aloe-budget-downgrade")}`,
    );
    assert(
      res2.headers.get("x-aloe-budget-mode") === "downgraded",
      `x-aloe-budget-mode is downgraded: ${res2.headers.get("x-aloe-budget-mode")}`,
    );
  ...
  // substituting it with free model would be deceptive regardless of task complexity.
  {
    ...
    assert(
      res2d.headers.get("X-ALOE-Budget-Mode") === "blocked",
      `X-ALOE-Budget-Mode is blocked: ${res2d.headers.get("X-ALOE-Budget-Mode")}`,
    );
  ...
  // Test 10c: maxCostPerRun graceful mode — tools request blocked when budget can't afford any model
  {
    ...
    assert(
      res.headers.get("X-ALOE-Budget-Mode") === "blocked",
      `X-ALOE-Budget-Mode is blocked: ${res.headers.get("X-ALOE-Budget-Mode")}`,
    );
