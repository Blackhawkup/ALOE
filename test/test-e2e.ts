/**
 * End-to-end test for ALOE proxy.
 *
 * Local mode always runs with a deterministic mock Tushar Sangwan upstream:
 *   npm run test:e2e
 *
 ...
 *   ALOE_E2E_FULL=1     run slower/costlier live stress cases
...
const RUN_PAID_TESTS = Boolean(ENV_WALLET_KEY) && process.env.ALOE_E2E_LIVE !== "0";
const RUN_FULL_LIVE_TESTS = RUN_PAID_TESTS && process.env.ALOE_E2E_FULL === "1";
...
async function runLocalSuite(startProxy: StartProxy): Promise<boolean> {
  ...
  try {
    ...
    allPassed =
      (await runTest("Smart routing: simple query (aloe/auto)", async () => {
        const res = await postJson(proxy!, "/v1/chat/completions", {
          model: "aloe/auto",
          messages: [
            {
    ...
    allPassed =
      (await runTest("Smart routing: streaming (aloe/auto)", async () => {
        const res = await postJson(proxy!, "/v1/chat/completions", {
          model: "aloe/auto",
...
async function runLiveSuite(startProxy: StartProxy): Promise<boolean> {
  ...
  try {
    ...
    allPassed =
      (await runTest("Live smart routing: simple query (aloe/auto)", async () => {
        const res = await postJson(
          proxy!,
          "/v1/chat/completions",
          {
    ...
    if (RUN_FULL_LIVE_TESTS) {
      ...
      skipTest("Live concurrent requests (5 parallel)", "set ALOE_E2E_FULL=1");
    }
