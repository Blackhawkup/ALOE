...
export default defineConfig({
  ...
  esbuildOptions(options) {
    // We and @aloe/llm depend on each other, and `noExternal` inlines it. Its
    // `import { route, ... } from "aloe"` therefore resolves through
    // node_modules to the LAST PUBLISHED build of this package, and esbuild inlines
    // that whole stale bundle beside the one it is building — a second ALOE,
  ...
  banner: {
    js: `import { createRequire as __aloe_createRequire } from 'node:module'; const require = __aloe_createRequire(import.meta.url);`,
  },
});
